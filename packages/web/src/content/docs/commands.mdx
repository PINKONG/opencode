...
Create `.wiscode/commands/test.md`:
...
You can add custom commands through the WisCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your WisCode [config](/docs/config):
...
WisCode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
