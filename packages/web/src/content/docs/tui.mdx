---
title: TUI
description: Using the WisCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WisCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running WisCode starts the TUI for the current directory.
...
When using the WisCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to WisCode. Allows you to select from available providers and add their API keys.
...
This is separate from `wiscode.json`, which configures server/runtime behavior.
