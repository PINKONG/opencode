---
title: Configuración
description: Usando la configuración WisCode JSON.
---

Puede configurar WisCode usando un archivo de configuración JSON.

---

## Formato

WisCode admite los formatos **JSON** y **JSONC** (JSON con comentarios).
...
2. **Configuración global** (`~/.config/wiscode/wiscode.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`wiscode.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.wiscode`** - agentes, comandos, complementos
...
:::note
Los directorios `.wiscode` y `~/.config/wiscode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global WisCode en `~/.config/wiscode/wiscode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `wiscode.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.

:::tip
Coloque la configuración específica del proyecto en la raíz de su proyecto.
:::

Cuando se inicia WisCode, busca un archivo de configuración en el directorio actual o recorre hasta el directorio Git más cercano.
...
modos y complementos como el directorio estándar `.wiscode`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.wiscode`, por lo que **puede anular** su configuración.
...
Puede configurar los ajustes del servidor para los comandos `wiscode serve` y `wiscode web` a través de la opción `server`.
...
- `mdns`: habilita el descubrimiento de servicios mDNS. Esto permite que otros dispositivos en la red descubran su servidor WisCode.
...
Puede configurar los proveedores y modelos que desea utilizar en su configuración WisCode a través de las opciones `provider`, `model` y `small_model`.
...
La opción `small_model` configura un modelo separado para tareas livianas como la generación de títulos. De forma predeterminada, WisCode intenta utilizar un modelo más económico si su proveedor tiene uno disponible; de ​​lo contrario, recurre a su modelo principal.
...
Puede configurar el tema que desea usar en su configuración WisCode a través de la opción `theme`.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/wiscode/agents/` o `.wiscode/agents/`. [Más información aquí](/docs/agents).
...
El agente predeterminado debe ser un agente principal (no un subagente). Puede ser un agente integrado como `"build"` o `"plan"`, o un [agente personalizado](/docs/agents) que haya definido. Si el agente especificado no existe o es un subagente, WisCode recurrirá a `"build"` con una advertencia.

Esta configuración se aplica en todas las interfaces: TUI, CLI (`wiscode run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/wiscode/commands/` o `.wiscode/commands/`. [Más información aquí](/docs/commands).
...
WisCode descargará automáticamente cualquier actualización nueva cuando se inicie. Puede desactivar esto con la opción `autoupdate`.
...
De forma predeterminada, WisCode **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Los [complementos](/docs/plugins) amplían WisCode con herramientas, enlaces e integraciones personalizados.

Coloque los archivos de complemento en `.wiscode/plugins/` o `~/.config/wiscode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
...
Esto es útil cuando desea restringir WisCode para que utilice únicamente proveedores específicos en lugar de deshabilitarlos uno por uno.
