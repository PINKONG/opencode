...
Para depurar problemas con WisCode, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/wiscode/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\wiscode\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `wiscode --log-level DEBUG`.

---

## Almacenamiento

WisCode almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\wiscode`
...
WisCode Desktop ejecuta un servidor WisCode local (el sidecar `wiscode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `WisCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/wiscode/wiscode.jsonc` (o `~/.config/wiscode/wiscode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\wiscode\wiscode.jsonc`
...
WisCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\wiscode\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.wiscode/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que WisCode pueda reconstruirlo.

1. Salga de WisCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/wiscode`
- **Linux**: eliminar `~/.cache/wiscode` (o ejecutar `rm -rf ~/.cache/wiscode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\wiscode`

3. Reinicie el escritorio WisCode.

---

### Solucionar problemas de conexión del servidor

WisCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `wiscode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio WisCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de WisCode.

---

### Notificaciones que no se muestran

WisCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para WisCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio WisCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio WisCode):
...
Si tiene problemas con WisCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/PINKONG/opencode/issues**](https://github.com/PINKONG/opencode/issues)
...
3. Asegúrese de tener la última versión con `wiscode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `wiscode models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/wiscode
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\wiscode`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. WisCode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\wiscode`

2. Reinicie WisCode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a WisCode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
WisCode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
