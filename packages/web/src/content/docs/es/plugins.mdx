---
title: Complementos
description: Escriba sus propios complementos para extender WisCode.
---

Los complementos le permiten extender WisCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de WisCode.
...
- `.wiscode/plugins/` - Complementos a nivel de proyecto
- `~/.config/wiscode/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/wiscode/node_modules/`.
...
1. Configuración global (`~/.config/wiscode/wiscode.json`)
2. Configuración del proyecto (`wiscode.json`)
3. Directorio global de complementos (`~/.config/wiscode/plugins/`)
4. Directorio de complementos del proyecto (`.wiscode/plugins/`)
...
WisCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK WisCode para interactuar con la IA.
...
:::note
Si está utilizando la aplicación de escritorio WisCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que WisCode lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a WisCode:
...
El ayudante `tool` crea una herramienta personalizada a la que WisCode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para WisCode junto con las herramientas integradas.
