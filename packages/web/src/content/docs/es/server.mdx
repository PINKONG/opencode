---
title: Servidor
description: Interactuar con el servidor WisCode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `wiscode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente WisCode puede usar.
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `wiscode`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `wiscode serve` como a `wiscode web`.
...
Cuando ejecuta `wiscode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor WisCode para interactuar con WisCode mediante programación.
:::

Esta arquitectura permite que WisCode admita múltiples clientes y le permite interactuar con WisCode mediante programación.

Puede ejecutar `wiscode serve` para iniciar un servidor independiente. Si tienes el
WisCode TUI ejecutándose, `wiscode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos WisCode [IDE](/docs/ide).
...
El servidor WisCode expone las siguientes API.
