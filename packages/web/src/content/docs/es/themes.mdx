...
Con WisCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, WisCode usa nuestro propio tema `wiscode`.
...
WisCode viene con varios temas integrados.
...
- Quiere que WisCode coincida con la apariencia de su terminal
...
WisCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/wiscode/themes/*.json` o `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.wiscode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.wiscode/themes/*.json`
