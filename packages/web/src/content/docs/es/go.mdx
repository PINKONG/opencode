...
WisCode Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

:::note
WisCode Go está actualmente en beta.
:::

Go funciona como cualquier otro proveedor en WisCode. Te suscribes a WisCode Go y
...
WisCode Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

WisCode Go funciona como cualquier otro proveedor en WisCode.

1. Inicias sesión en **<a href={console}>WisCode Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `WisCode Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a WisCode Go.
:::
...
WisCode Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de WisCode
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con WisCode.
