...
Las habilidades del agente permiten a WisCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
WisCode busca estas ubicaciones:

- Configuración del proyecto: `.wiscode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/wiscode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, WisCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.wiscode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.wiscode/skills/git-release/SKILL.md` así:
...
WisCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `wiscode.json`:
...
**Para agentes integrados** (en `wiscode.json`):
