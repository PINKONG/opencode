---
title: Proveedores
description: Usando cualquier proveedor LLM en WisCode.
---

import config from "../../../../config.mjs"
export const console = config.console

WisCode usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración WisCode.
...
en `~/.local/share/wiscode/auth.json`.

---

### Configuración

Puedes personalizar los proveedores a través de la sección `provider` en tu WisCode
configuración.
...
WisCode Zen es una lista de modelos proporcionados por el equipo WisCode que han sido
probado y verificado para funcionar bien con WisCode. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con WisCode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione WisCode y diríjase a [opencode.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en WisCode y su uso es completamente opcional.

---

## WisCode Go

WisCode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de WisCode que han sido
probado y verificado para funcionar bien con WisCode.

1. Ejecute el comando `/connect` en TUI, seleccione `WisCode Go` y diríjase a [opencode.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con WisCode:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta WisCode:
   ...
   Para una configuración persistente o específica del proyecto, utilice `wiscode.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en WisCode no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que WisCode funcione correctamente.
   :::
...
WisCode utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear WisCode
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `wiscode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `wiscode.json`:
...
Para utilizar su suscripción GitHub Copilot con WisCode:
...
Para utilizar Google Vertex AI con WisCode:
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de WisCode, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación WisCode como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación WisCode enumerada como una sesión separada.
...
Puede configurar WisCode para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar WisCode para usar modelos locales a través de LM Studio.
...
Puedes configurar WisCode para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para WisCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con WisCode:
...
3. Copie la clave API para usarla en WisCode.
...
6. **Importante**: Antes de usar modelos en la nube en WisCode, debe obtener la información del modelo localmente:
...
WisCode Zen es una lista de modelos probados y verificados proporcionada por el equipo WisCode. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>WisCode Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **WisCode Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave WisCode API.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con WisCode:
...
4. Cree o actualice su archivo `wiscode.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a WisCode comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `wiscode auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración WisCode y:
