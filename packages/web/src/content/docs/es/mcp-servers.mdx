...
Puede agregar herramientas externas a WisCode utilizando el _Model Context Protocol_ o MCP. WisCode admite servidores locales y remotos.
...
Puede definir servidores MCP en su [WisCode Config](https://opencode.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
WisCode maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, WisCode:
...
Si el servidor requiere autenticación, WisCode le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `wiscode mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, WisCode almacenará los tokens de forma segura en `~/.local/share/wiscode/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en WisCode, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración WisCode como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar WisCode a su cuenta Sentry.
