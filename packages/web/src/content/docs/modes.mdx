...
:::caution
Modes are now configured through the `agent` option in the WisCode config. The
`mode` option is now deprecated. [Learn more](/docs/agents).
:::

Modes in WisCode allow you to customize the behavior, tools, and prompts for different use cases.

It comes with two built-in modes: **build** and **plan**. You can customize
these or configure your own through the WisCode config.
...
WisCode comes with two built-in modes.
...
- `edit` - Cannot modify existing files, except for files located at `.wiscode/plans/*.md` to detail the plan itself
...
Configure modes in your `wiscode.json` config file:
...
both the global WisCode config and the project specific config.
...
Create mode files in `.wiscode/modes/` for project-specific modes or `~/.config/wiscode/modes/` for global modes:
