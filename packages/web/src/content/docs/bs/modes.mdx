...
ove ili konfigurirajte svoje putem WisCode config.
...
WisCode dolazi sa dva ugrađena načina rada.
...
- `edit` - Ne mogu modificirati postojeće fajlove, osim fajlova koji se nalaze na `.wiscode/plans/*.md` radi detaljiziranja samog plana
...
Konfigurirajte načine rada u svom `wiscode.json` konfiguracijskom fajlu:
...
- Globalno: `~/.config/wiscode/modes/`
- Projekat: `.wiscode/modes/`
...
Ako temperatura nije navedena, WisCode koristi podrazumijevane postavke specifične za model (obično 0 za većinu modela i 0.55 za Qwen modele).
...
Kreirajte fajlove načina u `.wiscode/modes/` za specifične načine rada ili `~/.config/wiscode/modes/` za globalne načine:
