...
Možete dodati vanjske alate u WisCode koristeći _Model Context Protocol_, ili MCP. WisCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [WisCode Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
WisCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, WisCode će:
...
Ako server zahtijeva autentifikaciju, WisCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `wiscode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, WisCode sigurno čuva tokene u `~/.local/share/wiscode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u WisCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz WisCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete WisCode sa vašim Sentry nalogom.
