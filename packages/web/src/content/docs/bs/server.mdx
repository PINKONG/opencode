---
title: Server
description: Komunicirajte s WisCode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `wiscode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji WisCode klijent moze koristiti.
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `wiscode`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `wiscode serve` i za `wiscode web`.
...
Kada pokrenete `wiscode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite WisCode server za programsku interakciju sa opencode-om.
:::

Ova arhitektura omogucava WisCode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `wiscode serve` da startate standalone server. Ako je WisCode TUI vec pokrenut, `wiscode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste WisCode [IDE](/docs/ide) pluginovi.
...
WisCode server izlaže sljedece API-je.
