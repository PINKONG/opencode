---
title: CLI
description: WisCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WisCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
wiscode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa WisCode.
...
Pokrenite WisCode terminalski korisnički interfejs.
...
WisCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte WisCode agentima.
...
Priključite terminal na već pokrenut WisCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
wiscode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim WisCode backend-om. Na primjer:
...
WisCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `wiscode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/wiscode/auth.json`.

```bash
wiscode auth login
```

Kada se WisCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u WisCode.
...
Pokrenite WisCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `wiscode serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| `--attach`   |        | Priključite na pokrenuti WisCode server (npr. http://localhost:4096)    |
...
Pokrenite WisCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
wiscode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti WisCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `wiscode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše WisCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili WisCode dijeljenog URL-a.

```bash
wiscode import <file>
```

Možete uvesti iz lokalne datoteke ili WisCode dijeljenog URL-a.
...
Pokrenite WisCode headless server sa web interfejsom.

```bash
wiscode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup WisCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `wiscode`).
...
Deinstalirajte WisCode i uklonite sve povezane datoteke.
...
Ažurira WisCode na najnoviju verziju ili određenu verziju.
...
WisCode CLI prihvata sljedeće globalne zastavice.
...
WisCode se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `wiscode`) |
