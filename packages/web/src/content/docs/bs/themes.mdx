...
U WisCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, WisCode koristi nasu `wiscode` temu.
...
WisCode dolazi sa vise ugradenih tema.
...
- Zele da WisCode odgovara izgledu njihovog terminala
...
WisCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/wiscode/themes/*.json` ili `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.wiscode/themes/*.json`
4. **Trenutni radni direktorij** - `./.wiscode/themes/*.json`
