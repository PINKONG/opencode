...
Agent skills omogucavaju WisCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
WisCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.wiscode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/wiscode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, WisCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.wiscode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.wiscode/skills/git-release/SKILL.md` ovako:
...
WisCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `wiscode.json`:
...
**Za ugradene agente** (u `wiscode.json`):
