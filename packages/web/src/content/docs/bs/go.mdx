...
WisCode Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

:::note
WisCode Go je trenutno u beta fazi.
:::

Go radi kao bilo koji drugi provajder u WisCode-u. Pretplatite se na WisCode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili WisCode.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa WisCode-om.
:::
...
WisCode Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

WisCode Go radi kao bilo koji drugi provajder u WisCode-u.

1. Prijavite se na **<a href={console}>WisCode Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `WisCode Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na WisCode Go.
:::
...
WisCode Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj WisCode konfiguraciji
...
Napravili smo WisCode Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz WisCode koristite i bilo kojeg drugog provajdera.
