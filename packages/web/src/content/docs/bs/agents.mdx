...
Postoje dvije vrste agenata u WisCode: primarni agenti i podagenti.
...
WisCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
WisCode dolazi sa dva ugrađena subagenta, **General** i **Explore**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

WisCode dolazi sa dva ugrađena primarna agenta i dva ugrađena subagenta.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `wiscode.json`:
...
- Globalno: `~/.config/wiscode/agents/`
- Po projektu: `.wiscode/agents/`
...
Ako temperatura nije navedena, WisCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu WisCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj WisCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [OpenCode Zen](/docs/zen), koristili biste `opencode/gpt-5.1-codex` za GPT 5.1 Codex.
...
:::tip
Pokrenite `wiscode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/PINKONG/opencode).
:::
