---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u WisCode.
---

import config from "../../../../config.mjs"
export const console = config.console

WisCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj WisCode konfiguraciji.
...
u `~/.local/share/wiscode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem WisCode
config.
...
WisCode Zen je lista modela koje je obezbedio WisCode tim koji su bili
testirano i potvrđeno da dobro radi sa WisCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa WisCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite WisCode i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u WisCode i potpuno je opcionalan za korištenje.

---

## WisCode Go

WisCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža WisCode tim i koji su testirani i verificirani da dobro rade s WisCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `WisCode Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s WisCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete WisCode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `wiscode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u WisCode.
:::
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi WisCode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju WisCode konfiguraciju.
...
da pohrani token u WisCode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
WisCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate WisCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`wiscode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `wiscode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s WisCode:
...
Za korištenje Google Vertex AI s WisCode:
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki WisCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki WisCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati WisCode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati WisCode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati WisCode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za WisCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s WisCode:
...
3. Kopirajte API ključ za korištenje u WisCode.
...
6. **Važno**: Prije upotrebe modela oblaka u WisCode, morate lokalno povući informacije o modelu:
...
WisCode Zen je lista testiranih i verifikovanih modela koju je obezbedio WisCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>WisCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **WisCode Zen**.

```txt
   /connect
```

3. Unesite svoj WisCode API ključ.
...
Također možete dodati dodatne modele putem vaše WisCode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše WisCode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju WisCode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
4. Kreirajte ili ažurirajte svoju `wiscode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju WisCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `wiscode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite WisCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj WisCode konfiguraciji.
