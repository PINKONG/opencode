---
title: Dodaci
description: Napišite vlastite dodatke za proširenje WisCode.
---

Dodaci vam omogućavaju da proširite WisCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja WisCode.
...
- `.wiscode/plugins/` - Dodaci na nivou projekta
- `~/.config/wiscode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/wiscode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/wiscode/wiscode.json`)
2. Konfiguracija projekta (`wiscode.json`)
3. Globalni direktorij dodataka (`~/.config/wiscode/plugins/`)
4. Direktorij dodataka projekta (`.wiscode/plugins/`)
...
WisCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje WisCode.
...
Spriječite WisCode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u WisCode:
...
Pomoćnik `tool` kreira prilagođeni alat koji WisCode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za WisCode zajedno sa ugrađenim alatima.
