...
To debug issues with WisCode, start by checking the logs and local data it stores on disk.
...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\wiscode\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `wiscode --log-level DEBUG`.

---

## Storage

WisCode stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\wiscode`
...
WisCode Desktop runs a local WisCode server (the `wiscode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `WisCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/wiscode/wiscode.jsonc` (or `~/.config/wiscode/wiscode.json`)
- **macOS/Linux** (older installs): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\wiscode\wiscode.jsonc`
...
WisCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:

- **Global plugins**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\wiscode\plugins`
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so WisCode can rebuild it.

1. Quit WisCode Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/wiscode`
- **Linux**: delete `~/.cache/wiscode` (or run `rm -rf ~/.cache/wiscode`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\wiscode`

3. Restart WisCode Desktop.

---

### Fix server connection issues

WisCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `wiscode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, WisCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with WisCode's features.

---

### Notifications not showing

WisCode Desktop only shows system notifications when:

- notifications are enabled for WisCode in your OS settings, and
...
1. Quit WisCode Desktop.
2. Find and delete these files (they live in the WisCode Desktop app data directory):
...
If you're experiencing issues with WisCode:
...
3. Ensure you have the latest version with `wiscode upgrade`
...
To figure out what models you have access to, run `wiscode models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\wiscode`
...
If you encounter API call errors, this may be due to outdated provider packages. WisCode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\wiscode`

2. Restart WisCode to reinstall the latest provider packages

This will force WisCode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
WisCode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
