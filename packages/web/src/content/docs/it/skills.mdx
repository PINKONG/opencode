...
Le skill degli agenti permettono a WisCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
WisCode cerca in queste posizioni:

- Config di progetto: `.wiscode/skills/<name>/SKILL.md`
- Config globale: `~/.config/wiscode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, WisCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.wiscode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.wiscode/skills/git-release/SKILL.md` cosi':
...
WisCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `wiscode.json`:
...
**Per agenti integrati** (in `wiscode.json`):
