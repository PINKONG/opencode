...
Puoi aggiungere strumenti esterni a WisCode usando il _Model Context Protocol_, o MCP. WisCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [WisCode Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
WisCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, WisCode:
...
Se il server richiede autenticazione, WisCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `wiscode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, WisCode memorizzera' i token in modo sicuro in `~/.local/share/wiscode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in WisCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione WisCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare WisCode al tuo account Sentry.
