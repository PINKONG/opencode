---
title: Plugin
description: Scrivi plugin per estendere WisCode.
---

I plugin ti permettono di estendere WisCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di WisCode.
...
- `.wiscode/plugins/` - plugin a livello progetto
- `~/.config/wiscode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/wiscode/node_modules/`.
...
1. Config globale (`~/.config/wiscode/wiscode.json`)
2. Config di progetto (`wiscode.json`)
3. Directory plugin globale (`~/.config/wiscode/plugins/`)
4. Directory plugin di progetto (`.wiscode/plugins/`)
...
WisCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di WisCode per interagire con l'AI.
...
:::note
Se usi l'app desktop di WisCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a WisCode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a WisCode:
...
L'helper `tool` crea uno strumento personalizzato che WisCode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in WisCode insieme agli strumenti integrati.
