---
title: Server
description: Interagisci con il server WisCode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `wiscode serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client opencode.
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `wiscode`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `wiscode serve` sia per `wiscode web`.
...
Quando esegui `wiscode` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server WisCode per interagire con WisCode in modo programmatico.
:::

Questa architettura permette a WisCode di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `wiscode serve` per avviare un server standalone. Se la TUI di WisCode e gia in esecuzione, `wiscode serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di WisCode.
...
Il server WisCode espone le seguenti API.
