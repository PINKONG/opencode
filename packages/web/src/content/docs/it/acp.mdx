---
title: Supporto ACP
description: Usa WisCode in qualunque editor compatibile con ACP.
---

WisCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare WisCode via ACP, configura l'editor in modo che esegua il comando `wiscode acp`.

Il comando avvia WisCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'WisCode' nel selettore agenti di AI Chat.
...
Per usare WisCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare WisCode come agente ACP per la chat.
...
WisCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione WisCode
