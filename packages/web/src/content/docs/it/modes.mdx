...
Le modalita in WisCode ti permettono di personalizzare comportamento, strumenti e prompt per casi d'uso diversi.
...
WisCode include due modalita integrate.
...
- `edit` - Non puo modificare file esistenti, tranne i file in `.wiscode/plans/*.md` per dettagliare il piano
...
Configura le modalita nel file di configurazione `wiscode.json`:
...
- Globale: `~/.config/wiscode/modes/`
- Progetto: `.wiscode/modes/`
...
Se non viene specificata alcuna temperatura, WisCode usa i default specifici del modello (tipicamente 0 per la maggior parte dei modelli, 0.55 per i modelli Qwen).
...
sia per la configurazione globale di WisCode sia per quella specifica del progetto.
...
Crea file di modalita in `.wiscode/modes/` per modalita specifiche del progetto oppure in `~/.config/wiscode/modes/` per modalita globali:
