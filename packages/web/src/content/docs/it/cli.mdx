---
title: CLI
description: Opzioni e comandi della CLI di WisCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di WisCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
wiscode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con WisCode in modo programmatico.
...
Avvia l'interfaccia testuale di WisCode (interfaccia utente da terminale).
...
La CLI di WisCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per WisCode.
...
Collega un terminale a un backend server di WisCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
wiscode attach [url]
```

Questo consente di usare la TUI con un backend WisCode remoto. Per esempio:
...
WisCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `wiscode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/wiscode/auth.json`.

```bash
wiscode auth login
```

Quando WisCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in WisCode.
...
Esegui WisCode in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `wiscode serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| `--attach`   |       | Attach a un server WisCode in esecuzione (es. http://localhost:4096)    |
...
Avvia un server WisCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
wiscode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di WisCode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `wiscode`).
...
Elenca tutte le sessioni WisCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni WisCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione WisCode.

```bash
wiscode import <file>
```

Puoi importare da un file locale o da un URL di condivisione WisCode.
...
Avvia un server WisCode headless con interfaccia web.

```bash
wiscode web
```

Avvia un server HTTP e apre un browser per accedere a WisCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `wiscode`).
...
Disinstalla WisCode e rimuove tutti i file correlati.
...
Aggiorna WisCode all'ultima versione o a una versione specifica.
...
La CLI di WisCode accetta i seguenti flag globali.
...
WisCode può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `wiscode`)     |
