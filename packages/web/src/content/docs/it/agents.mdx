...
In WisCode esistono due tipi di agenti: agenti primari e subagenti.
...
WisCode include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
WisCode include due subagenti integrati: **General** e **Explore**. Li vediamo sotto.

---

## Integrati

WisCode include due agenti primari integrati e due subagenti integrati.
...
Configura gli agenti nel file `wiscode.json`:
...
- Globale: `~/.config/wiscode/agents/`
- Per progetto: `.wiscode/agents/`
...
Se non specifichi una temperature, WisCode usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di WisCode sia per la config specifica del progetto.
...
L'ID modello nella configurazione WisCode usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCode Zen](/docs/zen), useresti `opencode/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `wiscode models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/PINKONG/opencode).
:::
