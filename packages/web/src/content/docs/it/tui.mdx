---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di WisCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WisCode fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire WisCode avvia la TUI per la directory corrente.
...
Quando usi la TUI di WisCode, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a WisCode. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da WisCode. _Alias_: `/quit`, `/q`
...
Questo è separato da `wiscode.json`, che configura il comportamento di server/runtime.
