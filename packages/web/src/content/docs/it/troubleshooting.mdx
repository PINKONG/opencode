...
Per diagnosticare problemi con WisCode, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\wiscode\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `wiscode --log-level DEBUG`.

---

## Archiviazione

WisCode salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\wiscode`
...
WisCode Desktop esegue in background un server locale di WisCode (il sidecar `wiscode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `WisCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\wiscode\wiscode.jsonc`
...
WisCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\wiscode\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che WisCode possa ricostruirla.

1. Chiudi completamente WisCode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/wiscode`
- **Linux**: elimina `~/.cache/wiscode` (oppure esegui `rm -rf ~/.cache/wiscode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\wiscode`

3. Riavvia WisCode Desktop.

---

### Risolvi problemi di connessione al server

WisCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `wiscode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, WisCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di WisCode.

---

### Notifiche non visualizzate

WisCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per WisCode nelle impostazioni del sistema operativo, e
...
1. Chiudi WisCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app WisCode Desktop):
...
Se riscontri problemi con WisCode:
...
Per capire a quali modelli hai accesso, esegui `wiscode models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\wiscode`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. WisCode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\wiscode`

2. Riavvia WisCode per reinstallare i pacchetti provider piu recenti

Questo forzera WisCode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
WisCode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
