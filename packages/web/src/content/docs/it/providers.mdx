---
title: Provider
description: Usa qualsiasi provider LLM in WisCode.
---

import config from "../../../../config.mjs"
export const console = config.console

WisCode usa [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) per supportare **oltre 75 provider LLM** e anche modelli locali.

Per aggiungere un provider devi:

1. Aggiungere le API key del provider con il comando `/connect`.
2. Configurare il provider nella configurazione di WisCode.
...
WisCode Zen e una lista di modelli forniti dal team WisCode, testati e
verificati per funzionare bene con WisCode. [Scopri di piu](/docs/zen).

:::tip
Se sei all'inizio, ti consigliamo di partire da WisCode Zen.
:::

1. Esegui `/connect` nella TUI, seleziona WisCode e vai su [opencode.ai/auth](https://opencode.ai/auth).
...
Funziona come qualsiasi altro provider in WisCode ed e totalmente opzionale.
...
Per usare Amazon Bedrock con WisCode:
...
2. **Configura l'autenticazione** usando uno dei seguenti metodi:
   ...
   ***
   ...
   Imposta una di queste variabili d'ambiente mentre esegui WisCode:
   ...
   ***
   ...
   Per configurazione specifica del progetto o persistente, usa `wiscode.json`:
...
:::info
L'uso dell'abbonamento Claude Pro/Max in WisCode non è ufficialmente supportato da [Anthropic](https://anthropic.com).
:::
...
2. Vai su [Azure AI Foundry](https://ai.azure.com/) e fai il deploy di un modello.
   ...
   :::note
   Il nome del deployment deve corrispondere al nome del modello affinché WisCode funzioni correttamente.
   :::
...
WisCode usa un modello piccolo per alcuni task AI come generare il titolo della sessione.
È configurato per usare gpt-5-nano di default, ospitato da Zen. Per bloccare WisCode
a usare solo la tua istanza GitLab self-hosted, aggiungi quanto segue al tuo
file `wiscode.json`. Si raccomanda anche di disabilitare la condivisione sessioni.
...
Personalizza tramite `wiscode.json`:
...
Per usare il tuo abbonamento GitHub Copilot con WisCode:
...
Per usare Google Vertex AI con WisCode:
...
Nel caso vedessi una funzionalità o un modello di Helicone che non viene configurato automaticamente da WisCode, puoi sempre configurarlo tu stesso.
...
La funzionalità [Sessions](https://docs.helicone.ai/features/sessions) di Helicone ti permette di raggruppare richieste LLM correlate. Usa il plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) per loggare automaticamente ogni conversazione di WisCode come una sessione in Helicone.
...
Il plugin inietta gli header `Helicone-Session-Id` e `Helicone-Session-Name` nelle tue richieste. Nella pagina Sessions di Helicone, vedrai ogni conversazione di WisCode elencata come sessione separata.
...
Puoi configurare WisCode per usare modelli locali tramite l'utility llama-server di [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puoi configurare WisCode per usare modelli locali tramite LM Studio.
...
Puoi configurare WisCode per usare modelli locali tramite Ollama.

:::tip
Ollama può auto-configurarsi per WisCode. Vedi la [documentazione di integrazione Ollama](https://docs.ollama.com/integrations/opencode) per dettagli.
:::
...
Per usare Ollama Cloud con WisCode:
...
3. Copia la chiave API da usare in WisCode.
...
6. **Importante**: Prima di usare modelli cloud in WisCode, devi scaricare le informazioni del modello localmente:
...
WisCode Zen è una lista di modelli testati e verificati forniti dal team WisCode. [Scopri di più](/docs/zen).

1. Accedi a **<a href={console}>WisCode Zen</a>** e clicca **Create API Key**.

2. Esegui il comando `/connect` e cerca **WisCode Zen**.

   ```txt
   /connect
   ```

3. Inserisci la tua chiave API di WisCode.
...
4. Crea o aggiorna il tuo file `wiscode.json` nella directory del progetto:
...
I campi `limit` permettono a WisCode di capire quanto contesto rimane. I provider standard recuperano questi dati da models.dev automaticamente.
...
1. **Controlla il setup auth**: Esegui `wiscode auth list` per vedere se le credenziali
...
2. Per provider personalizzati, controlla la config di WisCode e:
