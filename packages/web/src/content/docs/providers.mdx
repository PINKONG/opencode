---
title: Providers
description: Using any LLM provider in WisCode.
---

import config from "../../../config.mjs"
export const console = config.console

WisCode uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your WisCode config.
...
You can customize the providers through the `provider` section in your WisCode
config.
...
WisCode Zen is a list of models provided by the WisCode team that have been
tested and verified to work well with WisCode. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with WisCode Zen.
:::

1. Run the `/connect` command in the TUI, select `WisCode Zen`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in WisCode and is completely optional to use.

---

## WisCode Go

WisCode Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the WisCode team that have been
tested and verified to work well with WisCode.

1. Run the `/connect` command in the TUI, select `WisCode Go`, and head to [opencode.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with WisCode:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running WisCode:
   ...
   ***
   ...
   For project-specific or persistent configuration, use `wiscode.json`:
...
WisCode. Anthropic explicitly prohibits this.

Previous versions of WisCode came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in WisCode with zero setup:
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for WisCode to work properly.
   :::
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your WisCode config.
...
GitLab Duo support in WisCode is experimental. Features, configuration, and
behavior may change in future releases.
:::

WisCode integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
WisCode uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock WisCode
to only use your own GitLab-hosted instance, add the following to your
`wiscode.json` file. It is also recommended to disable session sharing.
...
Customize through `wiscode.json`:
...
When a `duo-workflow-*` model is selected, WisCode will:
...
4. Route tool execution requests through WisCode's permission-gated tool system
...
To use your GitHub Copilot subscription with WisCode:
...
To use Google Vertex AI with WisCode:
...
In the event you see a feature or model from Helicone that isn't configured automatically through WisCode, you can always configure it yourself.
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each WisCode conversation as a session in Helicone.
...
The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each WisCode conversation listed as a separate session.
...
You can configure WisCode to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure WisCode to use local models through LM Studio.
...
You can configure WisCode to use local models through Ollama.

:::tip
Ollama can automatically configure itself for WisCode. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::
...
To use Ollama Cloud with WisCode:
...
3. Copy the API key for use in WisCode.
...
6. **Important**: Before using cloud models in WisCode, you must pull the model information locally:
...
WisCode Zen is a list of tested and verified models provided by the WisCode team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>WisCode Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **WisCode Zen**.

   ```txt
   /connect
   ```

3. Enter your WisCode API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your WisCode config.
...
5. You can also customize them through your WisCode config. Here's an example of specifying a provider
...
You can also customize models through your WisCode config. Here's an example of specifying provider routing order.
...
4. Create or update your `wiscode.json` file in your project directory:
...
The `limit` fields allow WisCode to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `wiscode auth list` to see if the credentials
...
2. For custom providers, check the WisCode config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your WisCode config.
