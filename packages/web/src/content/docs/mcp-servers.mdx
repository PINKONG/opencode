...
You can add external tools to WisCode using the _Model Context Protocol_, or MCP. WisCode supports both local and remote servers.
...
You can define MCP servers in your [WisCode Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="wiscode.jsonc" {6}
{
...
| `auth`    | Object  |          | Advanced remote authentication for providers that require per-request signing.       |
| `timeout` | Number  |          | Timeout in ms for fetching tools from the MCP server. Defaults to 5000 (5 seconds). |
...
MaxKB exposes MCP over remote `streamable-http`, but it does not use static API-key headers or OAuth.

Instead, MaxKB requires WisCode to sign **every HTTP request** with:
...
Use the built-in MaxKB auth mode for this:

You must include `oauth: false` in the config. Omitting it will not work for MaxKB.
...
```json title="wiscode.json"
{
  ...
  "mcp": {
    "maxkb": {
      ...
      "auth": {
        "type": "maxkb-hmac",
        "appKey": "{env:MAXKB_APP_KEY}",
        "appSecret": "{env:MAXKB_APP_SECRET}"
      }
    }
  }
}
...
- `oauth` must be `false` for MaxKB. Do not combine `auth.type: "maxkb-hmac"` with OAuth config.
- Static `headers` are not enough for MaxKB because the signature depends on the exact request method, path, query string, and body.
- The MCP path must match exactly when signing. If your MaxKB server is mounted at `/mcp/`, do not configure it as `/mcp`.
- Keep `appSecret` in your user-level config or environment, not in project config checked into git.

MaxKB currently exposes browsing resources such as:

- `maxkb://datasets`
- `maxkb://datasets/{dataset_id}/documents`
- `maxkb://datasets/{dataset_id}/documents/{document_id}/paragraphs`

If MaxKB authentication fails, verify:

- the endpoint path is correct, including trailing slash
- the `appKey` and `appSecret` are current
- the local machine clock is accurate
- requests are not being replayed with the same nonce

---

## OAuth

WisCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, WisCode will:
...
If the server requires authentication, WisCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `wiscode mcp auth <server-name>`.
...
```bash
wiscode mcp auth my-oauth-server
```
...
```bash
wiscode mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, WisCode will store the tokens securely in `~/.local/share/wiscode/mcp-auth.json`.
...
wiscode mcp debug my-oauth-server
```
...
Your MCPs are available as tools in WisCode, alongside built-in tools. So you can manage them through the WisCode config like any other tool.
...
```json title="wiscode.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
wiscode mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect WisCode to your Sentry account.
