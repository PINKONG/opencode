...
Możesz dodać dodatkowe informacje do WisCode za pomocą _Model Context Protocol_, w skrócie MCP. WisCode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [WisCode Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
WisCode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, WisCode:
...
Jeśli serwer wymaga uwierzytelnienia, WisCode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `wiscode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji WisCode bezpieczne przechowa tokeny w `~/.local/share/wiscode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w WisCode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie WisCode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć WisCode z kontem Sentry.
