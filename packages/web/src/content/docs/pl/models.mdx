...
WisCode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługi uruchamiania modeli pierwszych.
...
Oto kilka modeli, które dobrze współpracują z WisCode, w kolejności. (Nie jest to lista wyczerpująca i nie jest konieczna aktualna):
...
Tutaj pełny identyfikator do `provider_id/model_id`. Na przykład, użycie [WisCode Zen](/docs/zen), przestrzeganie `opencode/gpt-5.1-codex` dla Kodeksu GPT 5.1.
...
Wiele modeli obsługuje wiele wariantów lub różnych wybranych. WisCode jest dostarczany z podstawowymi wariantami wariantów dla dostawców.

### Wbudowane warianty

WisCode jest dostarczany z domyślnymi wariantami dla wielu dostawców:
...
Po uruchomieniu WisCode sprawdzanie modeli w następującej kolejności:
