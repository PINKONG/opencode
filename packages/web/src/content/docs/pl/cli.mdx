---
title: CLI
description: Opcje i polecenia wiersza poleceń WisCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI WisCode domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
wiscode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać WisCode programowo.
...
Uruchamia interfejs terminalowy (TUI) WisCode.
...
Interfejs CLI WisCode zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami WisCode.
...
Dołącz terminal do już działającego serwera backendu WisCode uruchomionego za pomocą `serve` lub `web`.

```bash
wiscode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem WisCode. Na przykład:
...
WisCode integruje się z wieloma dostawcami. Możesz użyć `wiscode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/wiscode/auth.json`.

```bash
wiscode auth login
```

Podczas uruchamiania WisCode ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w WisCode.
...
Uruchom WisCode w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `wiscode serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| `--attach`   |       | Dołącz do działającego serwera WisCode (np. http://localhost:4096)     |
...
Uruchom serwer WisCode (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
wiscode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API WisCode bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `wiscode`).
...
Lista wszystkich sesji WisCode.
...
Pokaż statystyki wykorzystania tokenów i sesji WisCode.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania WisCode.
...
Uruchom serwer WisCode z interfejsem internetowym.

```bash
wiscode web
```

Uruchamia to serwer HTTP i udostępnia WisCode przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `wiscode`).
...
Odinstaluj WisCode i usuń wszystkie powiązane pliki.
...
Aktualizuj WisCode do najnowszej lub określonej wersji.
...
Interfejs CLI WisCode przyjmuje następujące flagi globalne dla każdego polecenia.
...
WisCode można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `wiscode`)     |
