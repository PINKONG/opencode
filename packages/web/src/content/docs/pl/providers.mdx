...
WisCode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.
...
w `~/.local/share/wiscode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim WisCode
config.
...
WisCode Zen to lista modeli dostarczonych przez zespół WisCode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z opencode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od WisCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `WisCode Zen` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w WisCode i jest całkowicie opcjonalny w użyciu.

---

## WisCode Go

WisCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół WisCode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z opencode.

1. Uruchom polecenie `/connect` w TUI, wybierz `WisCode Go` i przejdź do [opencode.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z WisCode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania WisCode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `wiscode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w WisCode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod WisCode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
WisCode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować WisCode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`wiscode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem WisCode:
...
Aby używać Google Vertex AI z WisCode:
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą WisCode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację WisCode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja WisCode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować WisCode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować WisCode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować WisCode tak, aby korzystał z modeli lokalnych poprzez Ollamę.
...
Aby korzystać z Ollama Cloud z WisCode:
...
6. **Ważne**: Przed użyciem modeli chmurowych w WisCode musisz pobrać informacje o modelu lokalnie:
...
WisCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół opencode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>WisCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **WisCode Zen**.
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z WisCode:
...
4. Utwórz lub zaktualizuj plik `wiscode.json` w katalogu projektu:
...
Pola `limit` pozwalają WisCode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `wiscode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację WisCode i:
