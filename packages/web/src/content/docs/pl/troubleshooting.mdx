...
Aby debugować problemy z WisCode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\wiscode\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `wiscode --log-level DEBUG`.

---

## Przechowywanie danych

WisCode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\wiscode`
...
WisCode Desktop uruchamia w tle lokalny serwer WisCode (sidecar `wiscode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `WisCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/wiscode/wiscode.jsonc` (lub `~/.config/wiscode/wiscode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\wiscode\wiscode.jsonc`
...
WisCode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\wiscode\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby WisCode mógł ją odbudować.

1. Całkowicie zamknij WisCode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/wiscode`
- **Linux**: usuń `~/.cache/wiscode` (lub uruchom `rm -rf ~/.cache/wiscode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\wiscode`

3. Uruchom ponownie WisCode Desktop.

---

### Napraw problemy z połączeniem z serwerem

WisCode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `wiscode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows WisCode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.
...
WisCode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla WisCode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie WisCode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji WisCode Desktop):
...
3. Upewnij się, że masz najnowszą wersję z `wiscode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `wiscode models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\wiscode`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. WisCode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\wiscode`

2. Uruchom ponownie kod WisCode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to WisCode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
WisCode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
