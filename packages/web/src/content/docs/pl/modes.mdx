...
- `edit` — Nie można zastosować naruszenia plików, z naruszeniem praw autorskich w `.wiscode/plans/*.md` w celu uszczegółowienia samego planu
...
Skonfiguruj tryb w pliku konfiguracyjnym `wiscode.json`:
...
- Globalnie: `~/.config/wiscode/modes/`
...
Jeśli nie ma wpływu na temperaturę, WisCode stosuje się narzędzia badawcze dla modelu (zwykle 0 dla największych modeli, 0,55 dla modeli Qwen).
...
zarówno globalna WisCode, jak i specjalna dla projektu.
...
Utwórz pliki trybów w `.wiscode/modes/` dla trybów zapisanych dla projektu lub `~/.config/wiscode/modes/` dla trybów globalnych:
