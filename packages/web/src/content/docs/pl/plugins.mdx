...
Wtyczki umożliwiające wyłączenie WisCode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania opencode.
...
- `.wiscode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/wiscode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/wiscode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/wiscode/wiscode.json`)
2. Konfiguracja projektu (`wiscode.json`)
3. Globalny katalog wtyczek (`~/.config/wiscode/plugins/`)
4. Katalog wtyczek projektu (`.wiscode/plugins/`)
...
WisCode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z WisCode do sztucznej inteligencji.
...
:::note
Jeśli korzystasz z aplikacji komputerowej WisCode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj WisCode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do WisCode:
