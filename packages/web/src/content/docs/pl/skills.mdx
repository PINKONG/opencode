...
Umiejętności agenta pozwalają WisCode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
WisCode przeszukuje te lokalizacje:

- Project config: `.wiscode/skills/<name>/SKILL.md`
- Global config: `~/.config/wiscode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu WisCode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.wiscode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.wiscode/skills/git-release/SKILL.md` w ten sposób:
...
WisCode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `wiscode.json`:
...
**Dla agentów wbudowanych** (w `wiscode.json`):
