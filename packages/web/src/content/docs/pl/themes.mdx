...
Dzięki WisCode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie WisCode używa naszego własnego motywu `wiscode`.
...
WisCode ma kilka wbudowanych motywów.
...
- Chcą, aby WisCode pasował do wyglądu ich terminala
...
WisCode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/wiscode/themes/*.json` lub `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.wiscode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.wiscode/themes/*.json`
