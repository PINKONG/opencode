---
title: Config
description: Using the WisCode JSON config.
---

You can configure WisCode using a JSON config file.

---

## Format

WisCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="wiscode.jsonc"
{
...
2. **Global config** (`~/.config/wiscode/wiscode.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`wiscode.json` in project) - project-specific settings
5. **`.wiscode` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/wiscode/` on macOS) - admin-controlled
...
:::note
The `.wiscode` and `~/.config/wiscode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Place your global WisCode config in `~/.config/wiscode/wiscode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/wiscode/tui.json`.
...
Add `wiscode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When WisCode starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
modes, and plugins just like the standard `.wiscode` directory, and should
...
The custom directory is loaded after the global config and `.wiscode` directories, so it **can override** their settings.
...
Drop an `wiscode.json` or `wiscode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/wiscode/` |
| Linux    | `/etc/wiscode/`                         |
| Windows  | `%ProgramData%\wiscode`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, WisCode reads managed preferences from the `ai.wiscode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

WisCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.wiscode.managed.plist`
2. `/Library/Managed Preferences/ai.wiscode.managed.plist`

The plist keys map directly to `wiscode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.wiscode.managed` PayloadType. The WisCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.wiscode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.wiscode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.wiscode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
Legacy `theme`, `keybinds`, and `tui` keys in `wiscode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `wiscode serve` and `wiscode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your WisCode server.
...
You can configure the providers and models you want to use in your WisCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, WisCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
You can also define agents using markdown files in `~/.config/wiscode/agents/` or `.wiscode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, WisCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`wiscode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/wiscode/commands/` or `.wiscode/commands/`. [Learn more here](/docs/commands).
...
WisCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
WisCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, WisCode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend WisCode with custom tools, hooks, and integrations.

Place plugin files in `.wiscode/plugins/` or `~/.config/wiscode/plugins/`. You can also load plugins from npm through the `plugin` option.
...
This is useful when you want to restrict WisCode to only use specific providers rather than disabling them one by one.
