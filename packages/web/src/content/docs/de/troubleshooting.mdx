...
Wenn WisCode Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\wiscode\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `wiscode --log-level DEBUG`.

---

## Speicher

WisCode speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\wiscode`
...
WisCode Desktop startet im Hintergrund einen lokalen WisCode-Server (`wiscode-cli`-Sidecar).
...
- Nur macOS: `WisCode`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/wiscode/wiscode.jsonc` (oder `~/.config/wiscode/wiscode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\wiscode\wiscode.jsonc`
...
WisCode kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\wiscode\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit WisCode ihn neu aufbauen kann.

1. Beende WisCode Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/wiscode`
- **Linux**: loesche `~/.cache/wiscode` (oder fuehre aus: `rm -rf ~/.cache/wiscode`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\wiscode`

3. Starte WisCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

WisCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `wiscode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
Unter Windows benoetigt WisCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit WisCode-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

WisCode Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer WisCode in den OS-Einstellungen aktiviert sind, und
...
1. Beende WisCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von WisCode Desktop):
...
Wenn du Probleme mit WisCode hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `wiscode models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\wiscode`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. WisCode installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\wiscode`

2. Starte WisCode neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt WisCode, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
WisCode erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
