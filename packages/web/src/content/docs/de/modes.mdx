...
:::caution
Modi werden jetzt über die Option `agent` in der WisCode-Konfiguration konfiguriert. Der
Die Option `mode` ist jetzt veraltet. [Learn more](/docs/agents).
:::

Mit den Modi in WisCode können Sie das Verhalten, die Tools und die Eingabeaufforderungen für verschiedene Anwendungsfälle anpassen.

Es verfügt über zwei integrierte Modi: **Build** und **Plan**. Sie können anpassen
diese oder konfigurieren Sie Ihre eigenen über die WisCode-Konfiguration.
...
WisCode verfügt über zwei integrierte Modi.
...
- `edit` – Vorhandene Dateien können nicht geändert werden, mit Ausnahme der Dateien unter `.wiscode/plans/*.md`, um den Plan selbst detailliert darzustellen
...
Konfigurieren Sie Modi in Ihrer `wiscode.json`-Konfigurationsdatei:
...
- Global: `~/.config/wiscode/modes/`
- Projekt: `.wiscode/modes/`

```markdown title="~/.config/wiscode/modes/review.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.1
tools:
...
Wenn keine Temperatur angegeben ist, verwendet WisCode modellspezifische Standardwerte (normalerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle).
...
sowohl die globale WisCode-Konfiguration als auch die projektspezifische Konfiguration.
...
Erstellen Sie Modusdateien in `.wiscode/modes/` für projektspezifische Modi oder `~/.config/wiscode/modes/` für globale Modi:

```markdown title=".wiscode/modes/debug.md"
---
temperature: 0.1
tools:
  bash: true
...
```markdown title="~/.config/wiscode/modes/refactor.md"
---
model: anthropic/claude-sonnet-4-20250514
temperature: 0.2
tools:
