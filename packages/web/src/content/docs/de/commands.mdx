...
Erstellen Sie `.wiscode/commands/test.md`:

```md title=".wiscode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die WisCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem WisCode [config](/docs/config):

```json title="wiscode.jsonc" {4-12}
{
...
- Global: `~/.config/wiscode/commands/`
- Pro Projekt: `.wiscode/commands/`

```markdown title="~/.config/wiscode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".wiscode/commands/component.md"
---
...
```md title=".wiscode/commands/create-file.md"
---
...
```md title=".wiscode/commands/analyze-coverage.md"
---
...
```md title=".wiscode/commands/review-changes.md"
---
...
```md title=".wiscode/commands/review-component.md"
---
...
WisCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
