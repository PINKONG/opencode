---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um WisCode zu erweitern.
---

Mit Plugins können Sie WisCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von WisCode zu ändern.
...
- `.wiscode/plugins/` – Plugins auf Projektebene
- `~/.config/wiscode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/wiscode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/wiscode/wiscode.json`)
2. Projektkonfiguration (`wiscode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/wiscode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.wiscode/plugins/`)
...
```json title=".wiscode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
WisCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.

```ts title=".wiscode/plugins/my-plugin.ts"
...
```js title=".wiscode/plugins/example.js"
...
- `client`: Ein WisCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie WisCode erweitern können.
...
```js title=".wiscode/plugins/notification.js"
...
:::note
Wenn Sie die WisCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass WisCode `.env`-Dateien liest:

```javascript title=".wiscode/plugins/env-protection.js"
...
```javascript title=".wiscode/plugins/inject-env.js"
...
Plugins können WisCode auch benutzerdefinierte Tools hinzufügen:

```ts title=".wiscode/plugins/custom-tools.ts"
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das WisCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für WisCode zur Verfügung.
...
```ts title=".wiscode/plugins/compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
...
```ts title=".wiscode/plugins/custom-compaction.ts"
import type { Plugin } from "@opencode-ai/plugin"
