---
title: CLI
description: WisCode CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der WisCode CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
wiscode
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit WisCode interagieren.
...
Starten Sie die Terminalbenutzeroberfläche WisCode.
...
Der WisCode CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für WisCode verwalten.
...
Schließen Sie ein Terminal an einen bereits laufenden WisCode-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
wiscode attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend WisCode. Zum Beispiel:
...
WisCode wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `wiscode auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/wiscode/auth.json` gespeichert.

```bash
wiscode auth login
```

Wenn WisCode startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in WisCode sehen möchten.
...
Führen Sie WisCode im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.
...
Sie können auch eine Verbindung zu einer laufenden `wiscode serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
| `--attach`   |      | An einen laufenden WisCode-Server anschließen (z.B. http://localhost:4096)                         |
...
Starten Sie einen Headless-WisCode-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
wiscode serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf WisCode-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `wiscode`).
...
Listen Sie alle WisCode-Sitzungen auf.
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre WisCode-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer WisCode-Freigabe-URL.

```bash
wiscode import <file>
```

Sie können aus einer lokalen Datei oder einer WisCode-Freigabe-URL importieren.
...
Starten Sie einen Headless-WisCode-Server mit einer Weboberfläche.

```bash
wiscode web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf WisCode zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `wiscode`).
...
Deinstallieren Sie WisCode und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert WisCode auf die neueste Version oder eine bestimmte Version.
...
Der WisCode CLI akzeptiert die folgenden globalen Flags.
...
WisCode kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `wiscode`) |
