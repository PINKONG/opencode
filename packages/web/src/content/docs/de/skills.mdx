...
Agent Skills erlauben WisCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
WisCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft WisCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.wiscode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.wiscode/skills/git-release/SKILL.md` zum Beispiel so:
...
WisCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `wiscode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `wiscode.json`):
