---
title: Regeln
description: Lege eigene Anweisungen fuer WisCode fest.
---

Du kannst WisCode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in WisCode den Befehl `/init` aus.
...
So kann WisCode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht WisCode sie zu erweitern.
...
WisCode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/wiscode/AGENTS.md` speichern.
...
Wenn du von Claude Code kommst, unterstuetzt WisCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/wiscode/AGENTS.md` existiert)
...
Beim Start sucht WisCode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/wiscode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `wiscode.json` oder in `~/.config/wiscode/wiscode.json` angeben.
...
WisCode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit wiscode.json

Empfohlen ist das `instructions`-Feld in der `wiscode.json`:
...
Du kannst WisCode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `wiscode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
