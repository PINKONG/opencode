---
title: WisCode Zen
description: WisCode によって提供されるモデルの厳選されたリスト。
---
...
WisCode Zen は、WisCode チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

:::note
WisCode Zen は現在ベータ版です。
:::

Zen は WisCode のほかのプロバイダーと同じように動作します。WisCode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、WisCode を使うために必須ではありません。
...
:::tip
WisCode でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
WisCode Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

WisCode Zen は、WisCode のほかのプロバイダーと同じように動作します。

1. **<a href={console}>WisCode Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、WisCode Zen を選んで API キーを貼り付けます。
...
WisCode 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.3 Codex では設定に `opencode/gpt-5.3-codex` を使用します。
...
- MiniMax M2.5 Free は期間限定で WisCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo V2 Pro Free は期間限定で WisCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo V2 Omni Free は期間限定で WisCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Qwen3.6 Plus Free は期間限定で WisCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Super Free は期間限定で WisCode で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で WisCode で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが WisCode Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに WisCode では常にほかのプロバイダーも使えるようにすること。
