...
Zen は WisCode のほかのプロバイダーと同じように動作します。OpenCode Zen にログインして API キーを取得できます。これは **完全にオプション** であり、WisCode を使うために必須ではありません。
...
OpenCode Zen は、WisCode のほかのプロバイダーと同じように動作します。
