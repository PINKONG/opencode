...
For at fejlfinde problemer med WisCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\wiscode\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `wiscode --log-level DEBUG`.

---

## Lagring

WisCode gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\wiscode`
...
WisCode Desktop kører en lokal WisCode-server (`wiscode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `WisCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/wiscode/wiscode.jsonc` (eller `~/.config/wiscode/wiscode.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\wiscode\wiscode.jsonc`
...
WisCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\wiscode\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så WisCode kan genopbygge den.

1. Afslut WisCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/wiscode`
- **Linux**: slet `~/.cache/wiscode` (eller kør `rm -rf ~/.cache/wiscode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\wiscode`

3. Start WisCode Desktop på ny.

---

### Løs problemer med serverforbindelse

WisCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `wiscode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver WisCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i WisCode.

---

### Meddelelser vises ikke

WisCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for WisCode i dine OS-indstillinger, og
...
1. Afslut WisCode Desktop.
2. Find og slet disse filer (de findes i WisCode Desktop-appens datamappe):
...
Hvis du oplever problemer med WisCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `wiscode models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\wiscode`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. WisCode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\wiscode`

2. Start WisCode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge WisCode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
WisCode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
