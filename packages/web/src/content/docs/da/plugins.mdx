---
title: Plugins
description: Skriv dine egne plugins for at udvide WisCode.
---

Plugins giver dig mulighed for at udvide WisCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenCodes standardadfærd.
...
- `.wiscode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/wiscode/node_modules/`.
...
1. Global konfiguration (`~/.config/wiscode/wiscode.json`)
2. Projektkonfiguration (`wiscode.json`)
3. Global plugin-mappe (`~/.config/wiscode/plugins/`)
4. Projekt plugin bibliotek (`.wiscode/plugins/`)
...
WisCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En WisCode SDK klient til interaktion med AI.
...
:::note
Hvis du bruger WisCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå WisCode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til WisCode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som WisCode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for WisCode sammen med indbyggede værktøjer.
