...
Du kan tilføje eksterne værktøjer til WisCode ved hjælp af _Model Context Protocol_ eller MCP. WisCode understøtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i din [WisCode Config](https://opencode.ai/docs/config/) under `mcp`. Tilføj hver MCP med et unikt navn. Du kan henvise til denne MCP ved navn, når du spørger LLM.
...
WisCode håndterer automatisk OAuth-godkendelse for eksterne MCP-servere. Når en server kræver godkendelse, vil WisCode:
...
Hvis serveren kræver godkendelse, vil WisCode bede dig om at godkende, når du prøver at bruge den første gang. Hvis ikke, kan du [manually trigger the flow](#authenticating) med `wiscode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åbner din browser for godkendelse. Når du har godkendt, vil WisCode opbevare tokens sikkert i `~/.local/share/wiscode/mcp-auth.json`.
...
Dine MCPs er tilgængelige som værktøjer i **TK\_** sammen med indbyggede værktøjer. Så du kan administrere dem gennem WisCode-konfigurationen som ethvert andet værktøj.
...
Dette åbner et browservindue for at fuldføre OAuth-flowet og forbinde WisCode til din Sentry-konto.
