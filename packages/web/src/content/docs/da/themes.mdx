...
Med WisCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger WisCode vårt eget `wiscode`-tema.
...
WisCode kommer med flere innebygde temaer.
...
- Vil at WisCode skal matche terminalens utseende
...
WisCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/wiscode/themes/*.json` eller `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.wiscode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.wiscode/themes/*.json`
