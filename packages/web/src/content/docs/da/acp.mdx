...
description: Brug WisCode i enhver ACP-kompatibel editor.
---

WisCode understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge WisCode via ACP, konfigurer din editor til at køre kommandoen `wiscode acp`.

Kommandoen starter WisCode som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'WisCode'-agent i AI Chat-agentvælgeren.
...
For at bruge WisCode som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge WisCode som ACP-agent til chat.
...
WisCode fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din WisCode-konfiguration
