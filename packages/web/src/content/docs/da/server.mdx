...
Kommandoen `wiscode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en opencode-klient kan bruge.
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `wiscode`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `wiscode serve` og `wiscode web`.
...
Når du kører `wiscode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug opencode-serveren til at interagere med WisCode programmatisk.
:::

Denne arkitektur lader WisCode understøtte flere klienter og lader dig interagere med WisCode programmatisk.

Du kan køre `wiscode serve` for at starte en selvstændig server. Hvis du har
WisCode TUI kørende, vil `wiscode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af WisCode [IDE](/docs/ide) plugins.
...
WisCode-serveren viser følgende API'er.
