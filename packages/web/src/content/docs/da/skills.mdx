...
Agentferdigheter lar WisCode oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
WisCode søker etter disse stedene:

- Prosjektkonfigurasjon: `.wiscode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/wiscode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går WisCode opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.wiscode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.wiscode/skills/git-release/SKILL.md` slik:
...
WisCode viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `wiscode.json`:
...
**For innebygde agenter** (i `wiscode.json`):
