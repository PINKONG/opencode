---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i WisCode.
---

import config from "../../../../config.mjs"
export const console = config.console

WisCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i WisCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/wiscode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i WisCode-konfigurationen.
...
WisCode Zen er en liste over modeller leveret af WisCode-teamet, som er blevet testet og verificeret til at fungere godt med WisCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med WisCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg WisCode og gå til [opencode.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i WisCode og er helt valgfrit at bruge.

---

## WisCode Go

WisCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af WisCode-teamet, som er testet og verificeret til at fungere godt med WisCode.

1. Kør kommandoen `/connect` i TUI, vælg `WisCode Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med WisCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører WisCode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `wiscode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i WisCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at WisCode skal fungere korrekt.
   :::
...
gemme tokenet i WisCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
WisCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse WisCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`wiscode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Sådan bruger du GitHub Copilot-abonnementet med WisCode:
...
Sådan bruger du Google Vertex AI med WisCode:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem WisCode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver WisCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver WisCode-samtale opført som en separat session.
...
Du kan konfigurere WisCode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere WisCode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere WisCode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for WisCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med WisCode:
...
3. Kopier API-nøglen til brug i WisCode.
...
6. **Vigtigt**: Før du bruger skymodeller i WisCode, skal du hente modelinformationen lokalt:
...
WisCode Zen er en liste over testede og verificerede modeller leveret af WisCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>WisCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **WisCode Zen**.

   ```txt
   /connect
   ```

3. Indtast WisCode API-nøglen.
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med WisCode:
...
4. Opret eller opdater `wiscode.json`-filen i projektkataloget:
...
`limit`-felterne lader WisCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `wiscode auth list` for at se om legitimationsoplysningerne
