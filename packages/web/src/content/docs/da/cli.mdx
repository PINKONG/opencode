---
title: CLI
description: WisCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WisCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
wiscode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med WisCode programmatisk.
...
Start WisCode terminalbrugergrænsefladen.
...
Tilslut en terminal til en allerede kørende WisCode backend-server startet via `serve` eller `web` kommandoer.

```bash
wiscode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern WisCode backend. For eksempel:
...
WisCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `wiscode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/wiscode/auth.json`.

```bash
wiscode auth login
```

Når WisCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i WisCode.
...
Kør WisCode i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `wiscode serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
Start en hovedløs WisCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
wiscode serve
```

Dette starter en HTTP-server, der giver API-adgang til opencode-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `wiscode`).
...
Administrator WisCode sessionsholder.
...
Liste over alle WisCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine WisCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller WisCode del URL.
...
Start en hovedløs WisCode-server med en webgrænseflade.

```bash
wiscode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til WisCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `wiscode`).
...
Afinstaller WisCode og fjern alle relaterede filer.
...
Opdaterer WisCode til den nyeste version eller en specifik version.
...
WisCode CLI tager følgende globale flag.
...
WisCode kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `wiscode`) |
