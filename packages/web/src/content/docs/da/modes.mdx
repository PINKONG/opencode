...
Tilstande i WisCode giver dig mulighed for at tilpasse adfærd, værktøjer og prompter til forskellige brugstilfælde.
...
WisCode leveres med to indbyggede tilstande.
...
- `edit` - Kan ikke ændre eksisterende filer, undtagen filer placeret på `.wiscode/plans/*.md` for at detaljere selve planen
...
Konfigurer tilstande i din `wiscode.json`-konfigurationsfil:
...
Hvis der ikke er angivet nogen temperatur, bruger WisCode modelspecifikke standarder (typisk 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Opret tilstandsfiler i `.wiscode/modes/` for projektspecifikke tilstande eller `~/.config/wiscode/modes/` for globale tilstande:
