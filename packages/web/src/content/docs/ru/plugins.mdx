...
Плагины позволяют расширять WisCode, подключаясь к различным событиям и настраивая поведение. Вы можете создавать плагины для добавления новых функций, интеграции с внешними сервисами или изменения поведения WisCode по умолчанию.
...
- `.wiscode/plugins/` – плагины уровня проекта.
- `~/.config/wiscode/plugins/` — глобальные плагины
...
**Плагины npm** устанавливаются автоматически с помощью Bun при запуске. Пакеты и их зависимости кэшируются в `~/.cache/wiscode/node_modules/`.
...
1. Глобальная конфигурация (`~/.config/wiscode/wiscode.json`)
2. Конфигурация проекта (`wiscode.json`)
3. Глобальный каталог плагинов (`~/.config/wiscode/plugins/`)
4. Каталог плагинов проекта (`.wiscode/plugins/`)
...
WisCode запускает `bun install` при запуске для их установки. Затем ваши плагины и инструменты смогут импортировать их.
...
:::note
Если вы используете настольное приложение WisCode, оно может автоматически отправлять системные уведомления, когда ответ готов или когда возникает ошибка сеанса.
:::
...
Плагины также могут добавлять в WisCode собственные инструменты:
