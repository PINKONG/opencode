...
Создайте `.wiscode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию WisCode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем WisCode [config](/docs/config):
...
- Глобальный: `~/.config/wiscode/commands/`
- Для каждого проекта: `.wiscode/commands/`
...
WisCode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
