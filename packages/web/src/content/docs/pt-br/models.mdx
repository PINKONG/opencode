...
O WisCode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.
...
Aqui estão vários modelos que funcionam bem com o WisCode, em nenhuma ordem específica. (Esta não é uma lista exaustiva nem necessariamente atualizada):
...
Aqui, o ID completo é `provider_id/model_id`. Por exemplo, se você estiver usando [WisCode Zen](/docs/zen), você usaria `opencode/gpt-5.1-codex` para GPT 5.1 Codex.
...
Muitos modelos suportam várias variantes com diferentes configurações. O WisCode vem com variantes padrão integradas para provedores populares.

### Variantes integradas

O WisCode vem com variantes padrão para muitos provedores:
...
Quando o WisCode é iniciado, ele verifica modelos na seguinte ordem de prioridade:
