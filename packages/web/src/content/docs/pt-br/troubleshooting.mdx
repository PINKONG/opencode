...
Para depurar problemas com o WisCode, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\wiscode\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `wiscode --log-level DEBUG`.

---

## Armazenamento

O WisCode armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\wiscode`
...
O WisCode Desktop executa um servidor WisCode local (o sidecar `wiscode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `WisCode` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\wiscode\wiscode.jsonc`
...
O WisCode também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\wiscode\plugins`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o WisCode possa reconstruí-lo.

1. Saia completamente do WisCode Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/wiscode`
- **Linux**: exclua `~/.cache/wiscode` (ou execute `rm -rf ~/.cache/wiscode`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\wiscode`

3. Reinicie o WisCode Desktop.

---

### Corrigindo problemas de conexão com o servidor

O WisCode Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `wiscode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o WisCode Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.
...
O WisCode Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o WisCode nas configurações do seu sistema operacional, e
...
1. Saia do WisCode Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo WisCode Desktop):
...
Se você estiver enfrentando problemas com o WisCode:
...
Para descobrir quais modelos você tem acesso, execute `wiscode models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\wiscode`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O WisCode instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\wiscode`

2. Reinicie o WisCode para reinstalar os pacotes de provedores mais recentes

Isso forçará o WisCode a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O WisCode detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
