...
Você pode adicionar ferramentas externas ao WisCode usando o _Model Context Protocol_, ou MCP. O WisCode suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do WisCode](https://opencode.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
O WisCode lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o WisCode irá:
...
Se o servidor requer autenticação, o WisCode solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `wiscode mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o WisCode armazenará os tokens de forma segura em `~/.local/share/wiscode/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no WisCode, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do WisCode como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o WisCode à sua conta do Sentry.
