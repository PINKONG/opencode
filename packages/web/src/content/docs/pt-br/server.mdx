---
title: Servidor
description: Interaja com o servidor WisCode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `wiscode serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente WisCode pode usar.
...
```bash
wiscode serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
```bash
wiscode serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `wiscode`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `wiscode serve` quanto ao `wiscode web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password wiscode serve
```
...
Quando você executa `wiscode`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor WisCode para interagir com o WisCode programaticamente.
:::

Essa arquitetura permite que o WisCode suporte múltiplos clientes e permite que você interaja com o WisCode programaticamente.

Você pode executar `wiscode serve` para iniciar um servidor autônomo. Se você tiver o TUI do WisCode em execução, `wiscode serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do WisCode [IDE](/docs/ide).
...
O servidor WisCode expõe as seguintes APIs.
