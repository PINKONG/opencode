...
O SDK JS/TS do WisCode fornece um cliente seguro em tipos para interagir com o servidor.
Use-o para construir integrações e controlar o WisCode programaticamente.
...
Você pode passar um objeto de configuração para personalizar o comportamento. A instância ainda pega seu `wiscode.json`, mas você pode substituir ou adicionar configuração inline:
...
Se você já tem uma instância do WisCode em execução, pode criar uma instância de cliente para se conectar a ela:
