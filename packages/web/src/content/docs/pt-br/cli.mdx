...
A CLI do WisCode, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
wiscode
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o WisCode programaticamente.
...
A CLI do WisCode também possui os seguintes comandos.
...
Anexe um terminal a um servidor backend do WisCode já em execução, iniciado via comandos `serve` ou `web`.

```bash
wiscode attach [url]
```

Isso permite usar o TUI com um backend WisCode remoto. Por exemplo:
...
O WisCode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `wiscode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/wiscode/auth.json`.

```bash
wiscode auth login
```

Quando o WisCode é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Execute o WisCode em modo não interativo passando um prompt diretamente.
...
```bash "opencode run"
wiscode run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `wiscode serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
wiscode run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| `--attach`   |       | Anexe a um servidor WisCode em execução (por exemplo, http://localhost:4096) |
...
Inicie um servidor WisCode sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
wiscode serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do WisCode sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `wiscode`).
...
Inicie um servidor WisCode sem cabeça com uma interface web.

```bash
wiscode web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o WisCode através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `wiscode`).
...
Desinstale o WisCode e remova todos os arquivos relacionados.
...
Atualiza o WisCode para a versão mais recente ou uma versão específica.
...
A CLI do WisCode aceita as seguintes flags globais.
...
O WisCode pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `wiscode`) |
