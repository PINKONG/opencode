...
Com o WisCode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o WisCode usa nosso próprio tema `wiscode`.
...
O WisCode vem com vários temas embutidos.
...
- Querem que o WisCode corresponda à aparência do seu terminal
...
O WisCode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/wiscode/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.wiscode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.wiscode/themes/*.json`
