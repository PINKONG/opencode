...
Os modos no WisCode permitem que você personalize o comportamento, as ferramentas e os prompts para diferentes casos de uso.
...
O WisCode vem com dois modos integrados.
...
- `edit` - Não pode modificar arquivos existentes, exceto para arquivos localizados em `.wiscode/plans/*.md` para detalhar o plano em si
...
Configure os modos no seu arquivo de configuração `wiscode.json`:
...
Se nenhuma temperatura for especificada, o WisCode usa padrões específicos do modelo (geralmente 0 para a maioria dos modelos, 0.55 para modelos Qwen).
...
Este caminho é relativo a onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do WisCode quanto para a configuração específica do projeto.
...
Crie arquivos de modo em `.wiscode/modes/` para modos específicos do projeto ou `~/.config/wiscode/modes/` para modos globais:
