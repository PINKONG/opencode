...
Você pode configurar o WisCode usando um arquivo de configuração JSON.

---

## Formato

O WisCode suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
2. **Configuração global** (`~/.config/wiscode/wiscode.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`wiscode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.wiscode`** - agentes, comandos, plugins
...
:::note
Os diretórios `.wiscode` e `~/.config/wiscode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do WisCode em `~/.config/wiscode/wiscode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.

Para configurações específicas do TUI, use `~/.config/wiscode/tui.json`.
...
Adicione `wiscode.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o WisCode é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.wiscode`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.wiscode`, então ele **pode substituir** suas configurações.
...
Chaves legadas `theme`, `keybinds` e `tui` em `wiscode.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `wiscode serve` e `wiscode web` através da opção `server`.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do WisCode através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o WisCode tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/wiscode/agents/` ou `.wiscode/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o WisCode recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`wiscode run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/wiscode/commands/` ou `.wiscode/commands/`. [Saiba mais aqui](/docs/commands).
...
O WisCode fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o WisCode **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o WisCode com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.wiscode/plugins/` ou `~/.config/wiscode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o WisCode para usar apenas provedores específicos, em vez de desabilitá-los um a um.
