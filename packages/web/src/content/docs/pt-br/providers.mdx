...
WisCode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.
...
Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/wiscode/auth.json`.
...
WisCode Zen é uma lista de modelos fornecidos pela equipe do WisCode que foram testados e verificados para funcionar bem com o opencode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o WisCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `WisCode Zen` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Funciona como qualquer outro provedor no WisCode e é completamente opcional.

---

## WisCode Go

WisCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do WisCode que foram
testados e verificados para funcionar bem com o opencode.

1. Execute o comando `/connect` no TUI, selecione `WisCode Go` e acesse [opencode.ai/zen](https://opencode.ai/zen).
...
Para usar o Amazon Bedrock com o WisCode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o WisCode:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `wiscode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no WisCode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o WisCode funcione corretamente.
   :::
...
WisCode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o WisCode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `wiscode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `wiscode.json`:
...
Para usar sua assinatura do GitHub Copilot com o WisCode:
...
Para usar o Google Vertex AI com o WisCode:
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do WisCode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do WisCode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do WisCode listada como uma sessão separada.
...
Você pode configurar o WisCode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o WisCode para usar modelos locais através do LM Studio.
...
Você pode configurar o WisCode para usar modelos locais através do Ollama.
...
Para usar o Ollama Cloud com o WisCode:
...
6. **Importante**: Antes de usar modelos em nuvem no WisCode, você deve puxar as informações do modelo localmente:
...
WisCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do opencode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>WisCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **WisCode Zen**.
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o WisCode:
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no wiscode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `wiscode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o WisCode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `wiscode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do WisCode e:
