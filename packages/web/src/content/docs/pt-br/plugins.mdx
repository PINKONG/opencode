...
Plugins permitem que você estenda o WisCode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do opencode.
...
- `.wiscode/plugins/` - Plugins em nível de projeto
- `~/.config/wiscode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/wiscode/node_modules/`.
...
1. Configuração global (`~/.config/wiscode/wiscode.json`)
2. Configuração do projeto (`wiscode.json`)
3. Diretório de plugins global (`~/.config/wiscode/plugins/`)
4. Diretório de plugins do projeto (`.wiscode/plugins/`)
...
O WisCode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do WisCode para interagir com a AI.
...
:::note
Se você estiver usando o aplicativo desktop WisCode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o WisCode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao WisCode:
...
O helper `tool` cria uma ferramenta personalizada que o WisCode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o WisCode junto com as ferramentas integradas.
