...
As habilidades do agente permitem que o WisCode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O WisCode pesquisa nesses locais:

- Configuração do projeto: `.wiscode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/wiscode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o WisCode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.wiscode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.wiscode/skills/git-release/SKILL.md` assim:
...
O WisCode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `wiscode.json`:
...
**Para agentes embutidos** (em `wiscode.json`):
