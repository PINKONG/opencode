...
Du kan legge til eksterne verktøy i WisCode ved å bruke _Model Context Protocol_ eller MCP. WisCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [WisCode Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.
...
WisCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil WisCode:
...
Hvis serveren krever autentisering, vil WisCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `wiscode mcp auth <server-name>`.
...
Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil WisCode lagre tokens sikkert i `~/.local/share/wiscode/mcp-auth.json`.
...
MCP-ene dine er tilgjengelige som verktøy i WisCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom WisCode-konfigurasjonen som alle andre verktøy.
...
Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble WisCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver WisCode-samtale oppført som en separat økt.
