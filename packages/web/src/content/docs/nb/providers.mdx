---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i WisCode.
---

import config from "../../../../config.mjs"
export const console = config.console

WisCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i WisCode-konfigurasjonen.
...
i `~/.local/share/wiscode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i WisCode
konfig.
...
WisCode Zen er en liste over modeller levert av WisCode-teamet som har vært
testet og verifisert for å fungere godt med WisCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med WisCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `WisCode Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i WisCode og er helt valgfritt å bruke.

---

## WisCode Go

WisCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av WisCode-teamet som har vært
testet og verifisert for å fungere godt med WisCode.

1. Kjør kommandoen `/connect` i TUI, velg `WisCode Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med WisCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører WisCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `wiscode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i WisCode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at WisCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom WisCode-konfigurasjonen.
...
for å lagre token i WisCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
WisCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse WisCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`wiscode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med WisCode:
...
Slik bruker du Google Vertex AI med WisCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører WisCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom WisCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver WisCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver WisCode-samtale oppført som en separat økt.
...
Du kan konfigurere WisCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere WisCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere WisCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for WisCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med WisCode:
...
3. Kopier API-nøkkelen for bruk i WisCode.
...
6. **Viktig**: Før du bruker skymodeller i WisCode, må du hente modellinformasjonen lokalt:
...
WisCode Zen er en liste over testede og verifiserte modeller levert av WisCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>WisCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **WisCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn WisCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom WisCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom WisCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med WisCode:
...
Du kan også tilpasse modeller gjennom WisCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med WisCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `wiscode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar WisCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `wiscode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk WisCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i WisCode-konfigurasjonen.
