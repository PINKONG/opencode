---
title: Konfigurasjon
description: Ved å bruke WisCode JSON-konfigurasjonen.
---

Du kan konfigurere WisCode ved å bruke en JSON konfigurasjonsfil.

---

## Format

WisCode støtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
2. **Global konfigurasjon** (`~/.config/wiscode/wiscode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`wiscode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.wiscode` kataloger** - agenter, kommandoer, plugins
...
:::note
`.wiscode` og `~/.config/wiscode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale WisCode-konfigurasjon i `~/.config/wiscode/wiscode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `wiscode.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.

:::tip
Plasser prosjektspesifikk konfigurasjon i roten til prosjektet ditt.
:::

Når WisCode starter opp, ser den etter en konfigurasjonsfil i gjeldende katalog eller går opp til nærmeste Git-katalog.
...
moduser og plugins akkurat som standard `.wiscode` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.wiscode`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan konfigurere serverinnstillinger for kommandoene `wiscode serve` og `wiscode web` gjennom alternativet `server`.
...
- `mdns` - Aktiver mDNS-tjenesteoppdagelse. Dette lar andre enheter på nettverket oppdage WisCode-serveren din.
...
Du kan konfigurere leverandørene og modellene du vil bruke i WisCode-konfigurasjonen gjennom alternativene `provider`, `model` og `small_model`.
...
Alternativet `small_model` konfigurerer en egen modell for lette oppgaver som tittelgenerering. Som standard prøver WisCode å bruke en rimeligere modell hvis en er tilgjengelig fra leverandøren din, ellers faller den tilbake til hovedmodellen din.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/wiscode/agents/` eller `.wiscode/agents/`. [Les mer her](/docs/agents).
...
Standardagenten må være en primæragent (ikke en underagent). Dette kan være en innebygd agent som `"build"` eller `"plan"`, eller en [egendefinert agent](/docs/agents) du har definert. Hvis den angitte agenten ikke eksisterer eller er en subagent, vil WisCode falle tilbake til `"build"` med en advarsel.

Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`wiscode run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/wiscode/commands/` eller `.wiscode/commands/`. [Les mer her](/docs/commands).
...
WisCode vil automatisk laste ned eventuelle nye oppdateringer når den starter opp. Du kan deaktivere dette med alternativet `autoupdate`.
...
Som standard **tillater WisCode alle operasjoner** uten å kreve eksplisitt godkjenning. Du kan endre dette ved å bruke alternativet `permission`.
...
[Plugins](/docs/plugins) utvider WisCode med tilpassede verktøy, kroker og integrasjoner.

Plasser plugin-filer i `.wiscode/plugins/` eller `~/.config/wiscode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
...
Dette er nyttig når du vil begrense WisCode til kun å bruke spesifikke leverandører i stedet for å deaktivere dem én etter én.
