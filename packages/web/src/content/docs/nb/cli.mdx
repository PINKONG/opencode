---
title: CLI
description: WisCode CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WisCode CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
wiscode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med WisCode programmatisk.
...
Start WisCode-terminalbrukergrensesnittet.
...
WisCode CLI har også følgende kommandoer.

---

### agent

Administrer agenter for WisCode.
...
Koble en terminal til en allerede kjørende WisCode-backend-server startet via `serve` eller `web` kommandoer.

```bash
wiscode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern WisCode-backend. For eksempel:
...
WisCode drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `wiscode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/wiscode/auth.json`.

```bash
wiscode auth login
```

Når WisCode starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i WisCode.
...
Kjør WisCode i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `wiscode serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| `--attach`   |      | Koble til en kjørende WisCode-server (f.eks. http://localhost:4096)     |
...
Start en headless WisCode-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
wiscode serve
```

Dette starter en HTTP-server som gir API tilgang til WisCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `wiscode`).
...
Vis tokenbruk og kostnadsstatistikk for WisCode-øktene dine.
...
Importer øktdata fra en JSON-fil eller en WisCode share-URL.

```bash
wiscode import <file>
```

Du kan importere fra en lokal fil eller en WisCode share-URL.
...
Start en headless WisCode-server med et webgrensesnitt.

```bash
wiscode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til WisCode via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `wiscode`).
...
Avinstaller WisCode og fjern alle relaterte filer.
...
Oppdaterer WisCode til siste versjon eller en spesifikk versjon.
...
WisCode CLI bruker følgende globale flagg.
...
WisCode kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `wiscode`) |
