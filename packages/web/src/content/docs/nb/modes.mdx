...
:::caution
Moduser konfigureres nå gjennom alternativet `agent` i WisCode-konfigurasjonen.
Alternativet `mode` er nå utdatert. [Finn ut mer](/docs/agents).
:::

Moduser i WisCode lar deg tilpasse oppførselen, verktøyene og prompter for ulike brukstilfeller.

Den kommer med to innebygde moduser: **bygg** og **plan**. Du kan tilpasse
disse eller konfigurer din egen gjennom WisCode-konfigurasjonen.
...
WisCode kommer med to innebygde moduser.
...
- `edit` - Kan ikke endre eksisterende filer, bortsett fra filer som ligger på `.wiscode/plans/*.md` for å detaljere selve planen
...
Konfigurer moduser i din `wiscode.json` konfigurasjonsfil:
...
- Globalt: `~/.config/wiscode/modes/`
- Prosjekt: `.wiscode/modes/`
...
Hvis ingen temperatur er spesifisert, bruker WisCode modellspesifikke standardinnstillinger (vanligvis 0 for de fleste modeller, 0,55 for Qwen-modeller).
...
Denne banen er relativ til der konfigurasjonsfilen er plassert. Så dette fungerer for både den globale WisCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Opprett modusfiler i `.wiscode/modes/` for prosjektspesifikke moduser eller `~/.config/wiscode/modes/` for globale moduser:
