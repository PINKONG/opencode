---
title: Tjener
description: Samhandle med WisCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `wiscode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en WisCode-klient kan bruke.
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `wiscode` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `wiscode serve` og `wiscode web`.
...
Når du kjører `wiscode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk WisCode-serveren til å samhandle med WisCode programmatisk.
:::

Denne arkitekturen lar WisCode støtte flere klienter og lar deg samhandle med WisCode programmatisk.

Du kan kjøre `wiscode serve` for å starte en frittstående server. Hvis du har
WisCode TUI kjører, vil `wiscode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av WisCode [IDE](/docs/ide) plugins.
