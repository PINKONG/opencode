...
description: Skriv dine egne programtillegg for å utvide WisCode.
---

Plugins lar deg utvide WisCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til WisCode.
...
- `.wiscode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/wiscode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/wiscode/wiscode.json`)
2. Prosjektkonfigurasjon (`wiscode.json`)
3. Global plugin-katalog (`~/.config/wiscode/plugins/`)
4. Prosjektpluginkatalog (`.wiscode/plugins/`)
...
WisCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En WisCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide WisCode.
...
:::note
Hvis du bruker WisCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre WisCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til WisCode:
...
`tool`-hjelperen lager et tilpasset verktøy som WisCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for WisCode sammen med innebygde verktøy.
