...
Det er to typer agenter i WisCode; primære agenter og underagenter.
...
WisCode kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
WisCode kommer med to innebygde underagenter, **General** og **Explore**. Vi skal se på dette nedenfor.

---

## Innebygd

WisCode kommer med to innebygde primære agenter og to innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `wiscode.json`:
...
- Globalt: `~/.config/wiscode/agents/`
- Per prosjekt: `.wiscode/agents/`
...
Hvis ingen temperatur er spesifisert, bruker WisCode modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale WisCode-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i WisCode-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenCode Zen](/docs/zen), vil du bruke `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `wiscode models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/PINKONG/opencode).
:::
