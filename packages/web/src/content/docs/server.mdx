---
title: Server
description: Interact with WisCode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `wiscode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an WisCode client can use.
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `wiscode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `wiscode serve` and `wiscode web`.
...
When you run `wiscode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the WisCode server to interact with WisCode programmatically.
:::

This architecture lets WisCode support multiple clients and allows you to interact with WisCode programmatically.

You can run `wiscode serve` to start a standalone server. If you have the
WisCode TUI running, `wiscode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the WisCode [IDE](/docs/ide) plugins.
...
The WisCode server exposes the following APIs.
