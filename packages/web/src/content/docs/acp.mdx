---
title: ACP Support
description: Use WisCode in any ACP-compatible editor.
---

WisCode supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use WisCode via ACP, configure your editor to run the `wiscode acp` command.

The command starts WisCode as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
To open it, use the new 'WisCode' agent in the AI Chat agent selector.
...
To use WisCode as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use WisCode as the ACP agent for chat.
...
WisCode works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your WisCode config
