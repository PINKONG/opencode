---
title: Serveur
description: Interagissez avec le serveur WisCode via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `wiscode serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client WisCode peut utiliser.
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `wiscode`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `wiscode serve` et à `wiscode web`.
...
Lorsque vous exécutez `wiscode`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur WisCode pour interagir avec WisCode par programmation.
:::

Cette architecture permet à WisCode de prendre en charge plusieurs clients et vous permet d'interagir avec WisCode par programme.

Vous pouvez exécuter `wiscode serve` pour démarrer un serveur autonome. Si vous avez le
WisCode TUI en cours d'exécution, `wiscode serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins WisCode [IDE](/docs/ide).
...
Le serveur WisCode expose les API suivantes.
