...
Il existe deux types d'agents dans WisCode ; agents primaires et sous-agents.
...
WisCode est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
WisCode est livré avec deux sous-agents intégrés, **General** et **Explore**. Nous verrons cela ci-dessous.

---

## Agents intégrés

WisCode est livré avec deux agents primaires intégrés et deux sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `wiscode.json` :
...
- Global : `~/.config/wiscode/agents/`
- Par projet : `.wiscode/agents/`

```markdown title="~/.config/wiscode/agents/review.md"
---
...
Si aucune température n'est spécifiée, WisCode utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale WisCode et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration WisCode utilise le format `provider/model-id`. Par exemple, si vous utilisez [OpenCode Zen](/docs/zen), vous utiliserez `opencode/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```json title="wiscode.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="wiscode.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Exécutez `wiscode models` pour voir une liste des modèles disponibles.
:::
...
:::tip
Avez-vous un agent que vous aimeriez partager ? [Soumettez un PR](https://github.com/PINKONG/opencode).
:::
...
```markdown title="~/.config/wiscode/agents/docs-writer.md"
---
...
```markdown title="~/.config/wiscode/agents/security-auditor.md"
---
