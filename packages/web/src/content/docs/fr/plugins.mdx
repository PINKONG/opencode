---
title: Plugins
description: Écrivez vos propres plugins pour étendre WisCode.
---

Les plugins vous permettent d'étendre WisCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de WisCode.
...
- `.wiscode/plugins/` - Plugins au niveau du projet
- `~/.config/wiscode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/wiscode/node_modules/`.
...
1. Configuration globale (`~/.config/wiscode/wiscode.json`)
2. Configuration du projet (`wiscode.json`)
3. Répertoire global des plugins (`~/.config/wiscode/plugins/`)
4. Répertoire des plugins du projet (`.wiscode/plugins/`)
...
WisCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK WisCode pour interagir avec l'IA.
...
:::note
Si vous utilisez l'application de bureau WisCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez WisCode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à WisCode :
...
L'assistant `tool` crée un outil personnalisé que WisCode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour WisCode aux côtés des outils intégrés.
