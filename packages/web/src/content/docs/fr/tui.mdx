---
title: TUI
description: Utilisation de l’interface utilisateur du terminal WisCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WisCode fournit une interface de terminal interactif ou TUI pour travailler sur vos projets avec un LLM.

L’exécution de WisCode démarre le TUI pour le répertoire actuel.
...
Lorsque vous utilisez le WisCode TUI, vous pouvez taper `/` suivi d'un nom de commande pour exécuter rapidement des actions. Par exemple:
...
Ajoutez un fournisseur à WisCode. Vous permet de sélectionner parmi les fournisseurs disponibles et d’ajouter leurs clés API.
...
Ceci est séparé de `wiscode.json`, qui configure le comportement du serveur/d'exécution.
