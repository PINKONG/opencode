...
Les modes dans WisCode vous permettent de personnaliser le comportement, les outils et les prompts pour différents cas d'utilisation.
...
WisCode est livré avec deux modes intégrés.
...
- `edit` - Impossible de modifier les fichiers existants, à l'exception des fichiers situés à `.wiscode/plans/*.md` pour détailler le plan lui-même
...
Configurez les modes dans votre fichier de configuration `wiscode.json` :
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Donc ça marche pour à la fois la configuration globale WisCode et la configuration spécifique au projet.
...
Créez des fichiers de mode dans `.wiscode/modes/` pour les modes spécifiques au projet ou `~/.config/wiscode/modes/` pour les modes globaux :
