...
Avec WisCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, WisCode utilise notre propre thème `wiscode`.
...
WisCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que WisCode corresponde à l'apparence de leur terminal
...
WisCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/wiscode/themes/*.json` ou `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.wiscode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.wiscode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/wiscode/themes
vim ~/.config/wiscode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .wiscode/themes
vim .wiscode/themes/my-theme.json
```
