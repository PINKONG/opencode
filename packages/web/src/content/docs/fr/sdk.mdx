...
Le SDK WisCode JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler WisCode par programme.
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `wiscode.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
Si vous disposez déjà d'une instance en cours d'exécution de WisCode, vous pouvez créer une instance client pour vous y connecter :
