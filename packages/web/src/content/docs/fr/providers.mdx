---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en WisCode.
---

import config from "../../../../config.mjs"
export const console = config.console

WisCode utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration WisCode.
...
Vous pouvez personnaliser les fournisseurs via la section `provider` de votre WisCode
configuration.
...
WisCode Zen est une liste de modèles fournis par l'équipe WisCode qui ont été
testé et vérifié pour fonctionner correctement avec WisCode. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par WisCode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez WisCode et dirigez-vous vers [opencode.ai/auth](https://opencode.ai/auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans WisCode et son utilisation est totalement facultative.

---

## WisCode Go

WisCode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe WisCode qui ont été
testé et vérifié pour fonctionner correctement avec WisCode.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `WisCode Go` et rendez-vous sur [opencode.ai/auth](https://opencode.ai/zen).
...
Pour utiliser Amazon Bedrock avec WisCode :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de WisCode :
...
Pour une configuration spécifique au projet ou persistante, utilisez `wiscode.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans WisCode n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que WisCode fonctionne correctement.
   :::
...
WisCode utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller WisCode
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `wiscode.json`. Il est également recommandé de désactiver le partage de session.
...
Personnalisez via `wiscode.json` :
...
Pour utiliser Google Vertex AI avec WisCode :
...
```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id WisCode
```
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [opencode-helicone-session](https://github.com/H2Shami/opencode-helicone-session) pour enregistrer automatiquement chaque conversation WisCode en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation WisCode répertoriée comme une session distincte.
...
:::tip
Ollama peut se configurer automatiquement pour WisCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/opencode) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec WisCode :
...
3. Copiez la clé API à utiliser dans WisCode.
...
6. **Important** : Avant d'utiliser des modèles cloud dans WisCode, vous devez extraire les informations du modèle localement :
...
WisCode Zen est une liste de modèles testés et vérifiés fournie par l'équipe WisCode. [En savoir plus](/docs/zen).

1. Connectez-vous à **<a href={console}>WisCode Zen</a>** et cliquez sur **Créer une clé API**.

2. Exécutez la commande `/connect` et recherchez **WisCode Zen**.

   ```txt
   /connect
   ```

3. Entrez votre clé WisCode API.
...
```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' WisCode
```
...
4. Définissez éventuellement l'ID de déploiement et le groupe de ressources :
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group WisCode
   ```
...
4. Créez ou mettez à jour votre fichier `wiscode.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à WisCode de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `wiscode auth list` pour voir si les informations d'identification
