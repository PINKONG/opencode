...
There are two types of agents in WisCode; primary agents and subagents.
...
WisCode comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
WisCode comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

WisCode comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `wiscode.json` config file:
...
If no temperature is specified, WisCode uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global WisCode config and the project specific config.
...
The model ID in your WisCode config uses the format `provider/model-id`. For example, if you're using [OpenCode Zen](/docs/zen), you would use `opencode/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `wiscode models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/PINKONG/opencode).
:::
