...
OpenCode Zen 是由 OpenCode 團隊提供的模型列表，這些模型已經過測試和驗證，能夠與 WisCode 良好配合使用。[了解更多](/docs/zen)。
...
OpenCode Go 是一個低成本的訂閱計畫，提供對 OpenCode 團隊提供的流行開放編碼模型的可靠存取，這些模型已經過測試和驗證，能夠與 WisCode 良好配合使用。
