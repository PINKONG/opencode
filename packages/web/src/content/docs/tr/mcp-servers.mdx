...
_Model Bağlam Protokolü_ veya MCP kullanarak WisCode'a harici araçlar ekleyebilirsiniz. WisCode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [WisCode Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
WisCode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde WisCode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa WisCode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `wiscode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından WisCode, belirteçleri `~/.local/share/wiscode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra WisCode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi WisCode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve WisCode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
