---
title: Eklentiler
description: WisCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek WisCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya WisCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.wiscode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/wiscode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/wiscode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/wiscode/wiscode.json`)
2. Project config (`wiscode.json`)
3. Global eklenti dizini (`~/.config/wiscode/plugins/`)
4. Proje eklenti dizini (`.wiscode/plugins/`)
...
WisCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir WisCode SDK istemcisi.
...
WisCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
WisCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
WisCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca WisCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, WisCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra WisCode'a da hazır olacaktır.
