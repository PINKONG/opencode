...
Ajan becerileri, WisCode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
WisCode su konumlari tarar:

- Proje konfigurasyonu: `.wiscode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/wiscode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda WisCode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.wiscode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/wiscode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.wiscode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
WisCode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`wiscode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`wiscode.json` icinde):
