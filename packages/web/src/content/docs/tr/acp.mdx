---
title: ACP Desteği
description: WisCode'u ACP uyumlu herhangi bir editörde kullanın.
---

WisCode, [Agent Client Protocol](https://agentclientprotocol.com) veya (ACP) dosyalarını destekleyerek onu doğrudan uyumlu editörlerde ve IDE'lerde kullanmanızı sağlar.
...
WisCode'u ACP aracılığıyla kullanmak için düzenleyicinizi `wiscode acp` komutunu çalıştıracak şekilde yapılandırın.

Komut, WisCode'u, editörünüzle stdio aracılığıyla JSON-RPC üzerinden iletişim kuran ACP uyumlu bir alt süreç olarak başlatır.
...
Açmak için AI Chat agent seçicisindeki yeni 'WisCode' ajanını kullanın.
...
WisCode'u [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)'de ACP ajanı olarak kullanmak için Neovim yapılandırmanıza aşağıdakileri ekleyin:
...
Bu yapılandırma, CodeCompanion'ı WisCode'u sohbet için ACP ajanı olarak kullanacak şekilde ayarlar.
...
WisCode, terminalde olduğu gibi ACP aracılığıyla aynı şekilde çalışır. Tüm özellikler desteklenir:
...
- WisCode yapılandırmanızda yapılandırılmış MCP sunucuları
