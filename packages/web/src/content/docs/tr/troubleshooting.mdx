...
WisCode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\wiscode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `wiscode --log-level DEBUG`.

---

## Depolama

WisCode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/wiscode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\wiscode` yapistirin
...
WisCode Desktop arka planda yerel bir WisCode sunucusu (`wiscode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `wiscode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/wiscode/wiscode.jsonc` (veya `~/.config/wiscode/wiscode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/wiscode/wiscode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\wiscode\wiscode.jsonc` yapistirin
...
WisCode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/wiscode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\wiscode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), WisCode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. WisCode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/wiscode`
- **Linux**: `~/.cache/wiscode` dizinini silin (veya `rm -rf ~/.cache/wiscode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\wiscode` yapistirin

3. WisCode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

WisCode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`wiscode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta WisCode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, WisCode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

WisCode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda WisCode icin bildirimler etkinse
...
1. WisCode Desktop'u kapatin
2. Su dosyalari bulun ve silin (WisCode Desktop uygulama veri dizininde yer alirlar):
...
WisCode ile ilgili bir sorun yasiyorsaniz:
...
3. `wiscode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `wiscode models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\wiscode`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. WisCode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\wiscode`

2. En guncel provider paketlerini yeniden kurmak icin WisCode'u yeniden baslatin

Bu, WisCode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
WisCode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
