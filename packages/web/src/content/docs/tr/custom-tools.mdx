---
title: Özel Araçlar
description: LLM'un WisCode'da çağırabileceği araçları birleştirebilir.
---

Özel araçlar, LLM'un konuşmalar sırasında arayabileceği, oluşturduğunuz işlevlerdir. WisCode'un [built-in tools](/docs/tools) ile birlikte `read`, `write` ve `bash` gibi çalışırlar.
...
- Yerel olarak bunları projenizin `.wiscode/tools/` dizinine yerleştirerek.
- Veya küresel olarak bunları `~/.config/wiscode/tools/` içine yerleştirerek.
...
```ts title=".wiscode/tools/database.ts" {1}
...
```ts title=".wiscode/tools/math.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".wiscode/tools/bash.ts"
import { tool } from "@opencode-ai/plugin"
...
```ts title=".wiscode/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".wiscode/tools/add.py"
import sys
...
```ts title=".wiscode/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".wiscode/tools/add.py")
