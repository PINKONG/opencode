...
WisCode Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

:::note
WisCode Go şu anda beta olarak mevcut.
:::

Go, WisCode'daki diğer sağlayıcılar gibi çalışır. WisCode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve WisCode'u kullanmak için bunu kullanmanıza gerek yoktur.
...
WisCode Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

WisCode Go, WisCode'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>WisCode Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `WisCode Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye WisCode Go'ya abone olabilir.
:::
...
WisCode Go aşağıdaki limitleri içerir:
...
WisCode yapılandırmanızdaki [model id](/docs/config/#models) formatı `opencode-go/<model-id>` şeklindedir. Örneğin, Kimi K2.5 için yapılandırmanızda `opencode-go/kimi-k2.5` kullanmalısınız.
...
WisCode Go'yu şu amaçlarla oluşturduk:
...
4. WisCode ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
