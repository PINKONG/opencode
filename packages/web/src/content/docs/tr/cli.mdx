---
title: CLI
description: WisCode CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

WisCode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
wiscode
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, WisCode ile programlı olarak etkileşim kurmanıza olanak tanır.
...
WisCode terminal kullanıcı arayüzünü başlatır.
...
WisCode CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

WisCode için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir WisCode arka uç sunucusuna bir terminal ekleyin.

```bash
wiscode attach [url]
```

Bu, TUI öğesinin uzak bir WisCode arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
WisCode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `wiscode auth login` kullanabilirsiniz. Bu `~/.local/share/wiscode/auth.json`'de saklanıyor.

```bash
wiscode auth login
```

WisCode başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları WisCode'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek WisCode'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `wiscode serve` örneğine de ekleyebilirsiniz:
...
| `--attach`   |      | Çalışan bir WisCode sunucusuna ekleyin (ör. http://localhost:4096)               |
...
API erişimi için headless bir WisCode sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
wiscode serve
```

Bu, TUI arayüzü olmadan WisCode işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `wiscode` şeklindedir).
...
Tüm WisCode oturumlarını listeleyin.
...
WisCode oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya WisCode paylaşımından URL oturum verilerini içe aktarın.

```bash
wiscode import <file>
```

Yerel bir dosyadan veya WisCode paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir WisCode sunucusu başlatın.

```bash
wiscode web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla WisCode'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `wiscode` şeklindedir).
...
WisCode'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
WisCode'u en son sürüme veya belirli bir sürüme günceller.
...
WisCode CLI aşağıdaki global bayrakları destekler.
...
WisCode ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `wiscode`) |
