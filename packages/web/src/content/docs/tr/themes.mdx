...
WisCode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak WisCode kendi `wiscode` temasını kullanır.
...
WisCode birden fazla yerleşik temayla gelir.
...
- WisCode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
WisCode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/wiscode/themes/*.json` veya `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.wiscode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.wiscode/themes/*.json`
