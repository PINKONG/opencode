...
:::caution
Modlar artık WisCode yapılandırmasındaki `agent` seçeneği aracılığıyla yapılandırılıyor.
`mode` seçeneği artık kullanımdan kaldırıldı. [Daha fazla bilgi](/docs/agents).
:::

WisCode'daki modlar, farklı kullanım durumları için davranışı, araçları ve istemleri özelleştirmenize olanak tanır.

İki yerleşik modla birlikte gelir: **build** ve **plan**. Kişiselleştirebilirsiniz
bunları veya WisCode yapılandırması aracılığıyla kendinizinkini yapılandırın.
...
WisCode iki yerleşik modla birlikte gelir.
...
- `edit` - Planın kendisini detaylandırmak için `.wiscode/plans/*.md` adresinde bulunan dosyalar dışında mevcut dosyalar değiştirilemez
...
`wiscode.json` yapılandırma dosyanızdaki modları yapılandırın:
...
Sıcaklık belirtilmezse, WisCode modele özgü varsayılanları kullanır (çoğu model için genellikle 0, Qwen modelleri için 0,55).
...
hem global WisCode yapılandırması hem de projeye özel yapılandırma.
...
Projeye özel modlar için `.wiscode/modes/` veya genel modlar için `~/.config/wiscode/modes/`'de mod dosyaları oluşturun:
