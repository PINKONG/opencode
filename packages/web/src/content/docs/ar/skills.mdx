...
تتيح مهارات الوكيل لـ WisCode اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث WisCode في هذه المواقع:

- إعدادات المشروع: `.wiscode/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/wiscode/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج WisCode صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.wiscode/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/wiscode/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.wiscode/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض WisCode المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `wiscode.json`:
...
**للوكلاء المدمجين** (في `wiscode.json`):
