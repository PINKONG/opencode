---
title: CLI
description: أوامر وخيارات CLI في WisCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ WisCode افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
wiscode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع WisCode برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ WisCode.
...
يتضمن CLI الخاص بـ WisCode أيضا الأوامر التالية.
...
إرفاق terminal بخادم الواجهة الخلفية لـ WisCode قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
wiscode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ WisCode تعمل عن بعد. على سبيل المثال:
...
يعتمد WisCode على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `wiscode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/wiscode/auth.json`.

```bash
wiscode auth login
```

عند تشغيل WisCode يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في WisCode.
...
تشغيل WisCode في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `wiscode serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| `--attach`   |         | الإرفاق بخادم WisCode قيد التشغيل (مثل http://localhost:4096)    |
...
بدء خادم WisCode بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
wiscode serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف WisCode دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `wiscode`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات WisCode لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة WisCode.

```bash
wiscode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة WisCode.
...
بدء خادم WisCode بدون واجهة مع واجهة ويب.

```bash
wiscode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى WisCode عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `wiscode`).
...
إلغاء تثبيت WisCode وإزالة جميع الملفات ذات الصلة.
...
تحديث WisCode إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر WisCode الخيارات العامة التالية.
...
يمكن تهيئة WisCode باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `wiscode`) |
