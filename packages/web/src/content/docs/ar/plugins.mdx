---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع WisCode.
---

تتيح لك الإضافات توسيع WisCode عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ WisCode.
...
- `.wiscode/plugins/` - إضافات على مستوى المشروع
- `~/.config/wiscode/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/wiscode/node_modules/`.
...
1. إعدادات عامة (`~/.config/wiscode/wiscode.json`)
2. إعدادات المشروع (`wiscode.json`)
3. دليل الإضافات العام (`~/.config/wiscode/plugins/`)
4. دليل إضافات المشروع (`.wiscode/plugins/`)
...
يشغّل WisCode الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ WisCode للتفاعل مع الذكاء الاصطناعي.
...
:::note
إذا كنت تستخدم تطبيق WisCode لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع WisCode من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى WisCode:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ WisCode استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ WisCode إلى جانب الأدوات المضمنة.
