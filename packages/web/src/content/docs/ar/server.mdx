---
title: الخادم
description: تفاعل مع خادم WisCode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `wiscode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل WisCode استخدامها.
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `wiscode`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `wiscode serve` و `wiscode web`.
...
عند تشغيل `wiscode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم WisCode للتفاعل مع WisCode برمجيا.
:::

تتيح هذه البنية لـ WisCode دعم عدة عملاء وتمكّنك من التفاعل مع WisCode برمجيا.

يمكنك تشغيل `wiscode serve` لبدء خادم مستقل. إذا كانت واجهة WisCode في terminal (TUI)
قيد التشغيل، فسيبدأ `wiscode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات WisCode لـ [IDE](/docs/ide).
...
يعرض خادم WisCode واجهات API التالية.
