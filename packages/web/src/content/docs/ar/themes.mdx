...
مع WisCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم WisCode سمتنا `wiscode`.
...
يأتي WisCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق WisCode مظهر terminal لديهم
...
يدعم WisCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/wiscode/themes/*.json` أو `$XDG_CONFIG_HOME/opencode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.wiscode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.wiscode/themes/*.json`
