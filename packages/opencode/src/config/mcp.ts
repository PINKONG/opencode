...
export const Remote = z
  .object({
    ...
    auth: MaxkbHmacAuth.optional().describe("Authentication configuration for the MCP server."),
    oauth: z
  ...
  .superRefine((input, ctx) => {
    if (input.auth?.type !== "maxkb-hmac") return
    if (input.oauth !== false) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        message: "maxkb-hmac auth requires oauth to be explicitly set to false",
        path: ["oauth"],
      })
    }
  })
