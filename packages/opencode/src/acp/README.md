...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for WisCode.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal WisCode sessions
...
wiscode acp --cwd /path/to/project
```
...
```bash
OPENCODE_ENABLE_QUESTION_TOOL=1 wiscode acp
```
...
- **Terminal Integration**: Full terminal support via WisCode's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | wiscode acp
```
...
ACP sessions map cleanly to WisCode's internal session model:
