...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run wiscode",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for WisCode session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found WisCode session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["wiscode", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting WisCode...")
        UI.println()

        const cmd = sessionId ? ["-s", sessionId] : []
        const proc = Process.spawn(["wiscode", ...cmd], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        ...
        if (code !== 0) throw new Error(`wiscode exited with code ${code}`)
      },
    })
  },
})
